-- =====================================================================================
-- BASELINE MIGRATION — verified current PRODUCTION application schema
-- Project: fmrsfjxwswzhvicylaph (Draw in the Air, production)
-- Generated: 2026-09-08 from LIVE catalog definitions (pg_get_functiondef, pg_get_constraintdef,
--            pg_get_indexdef, pg_get_triggerdef, pg_get_viewdef, pg_policies, aclexplode, cron.job).
-- Scope: application-owned objects only — schemas public + app_private, app-owned triggers on
--        auth.users, realtime publication membership, cron jobs. Supabase-managed schemas
--        (auth, storage, realtime, vault, extensions internals) are deliberately NOT included.
-- Postgres: 17.6. Verified equivalent to production by full object diff (docs/audits/evidence/release/WP1A.3/diff.md).
--
-- THIS FILE IS NOT HISTORICAL SQL. It is a reconstruction of the end state on 2026-09-08 (WP1A.3 method
-- step 4). It must be marked APPLIED on production via `supabase migration repair --status applied 20260908000000`
-- and NEVER executed against production. Executing it on an empty database reproduces production's
-- application schema; the diff of staging (built this way) vs production is on file.
--
-- Manual post-steps on a fresh database (cannot be represented as schema):
--   * INSERT INTO app_private.secrets(name, value) VALUES ('email_cron_key', <secret>);   -- never commit the value
--   * pricing_config / stripe_price_map configuration rows (prod: 1 and 4 rows) are data, not schema.
--   * Cron jobs' net.http_post URLs are the production ref; on a non-production project replace
--     fmrsfjxwswzhvicylaph with the target project ref (staging uses dcivdrhxeaiulbbhsgfv).
-- Known production quirk carried verbatim: cron job 'refresh-materialized-views' references
--   materialized views that do not exist in production (logged as a finding, not fixed here).
-- =====================================================================================
...
-- ---------- tables (columns, defaults, identity, generated, RLS, comments) ----------
...
COMMENT ON COLUMN public.class_children.first_name IS 'DEPRECATED / PII: do not collect real first names. Use display_name + avatar_seed. Pending erasure per Decision D3 after legal sign-off.';
COMMENT ON COLUMN public.class_children.notes IS 'PII RISK: free text. Review for personal data; consider removing/locking down per Decision D3.';

CREATE TABLE IF NOT EXISTS public.class_session_tokens (
  id uuid DEFAULT gen_random_uuid() NOT NULL,
  session_id uuid NOT NULL,
  class_child_id uuid NOT NULL,
  token text NOT NULL,
  created_at timestamp with time zone DEFAULT now() NOT NULL,
  claimed_at timestamp with time zone,
  claimed_session_student_id uuid,
  tenant_id uuid
);
ALTER TABLE public.class_session_tokens ENABLE ROW LEVEL SECURITY;
COMMENT ON TABLE public.class_session_tokens IS 'P3b (§11): per-session teacher-assigned picture tokens mapping to persistent class_children. Never exposed to anon; child joins by confirming one token.';
...
CREATE TABLE IF NOT EXISTS public.session_students (
  ...
  tenant_id uuid,
  class_child_id uuid,
  readiness_state text DEFAULT 'joined'::text NOT NULL,
  readiness_changed_at timestamp with time zone DEFAULT now() NOT NULL
);
ALTER TABLE public.session_students ENABLE ROW LEVEL SECURITY;
COMMENT ON COLUMN public.session_students.readiness_state IS 'Authoritative learner readiness (Decision DM2), separate from learning performance. Set idempotently by the join/conduct flow (P3).';
COMMENT ON COLUMN public.session_students.readiness_changed_at IS 'Timestamp of last readiness_state change; maintained by the P3 setter RPC.';
...
-- ---------- functions (140; pg_get_functiondef verbatim) ----------
...
-- FUNCTION public._check_active_has_activity()
...
-- FUNCTION public._class_assert_teacher(in_session_id uuid)
...
-- FUNCTION public._ensure_tenant(in_kind text, in_owner uuid, in_name text)
CREATE OR REPLACE FUNCTION public._ensure_tenant(in_kind text, in_owner uuid, in_name text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- FUNCTION public._get_child_skills(p_child uuid)
CREATE OR REPLACE FUNCTION public._get_child_skills(p_child uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public._stamp_child_tenant()
CREATE OR REPLACE FUNCTION public._stamp_child_tenant()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public._stamp_parent_profile_tenant()
CREATE OR REPLACE FUNCTION public._stamp_parent_profile_tenant()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public._stamp_parent_tenant()
CREATE OR REPLACE FUNCTION public._stamp_parent_tenant()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public._stamp_session_tenant()
CREATE OR REPLACE FUNCTION public._stamp_session_tenant()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public._stamp_teacher_tenant()
CREATE OR REPLACE FUNCTION public._stamp_teacher_tenant()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.analytics_events_promote_meta()
CREATE OR REPLACE FUNCTION public.analytics_events_promote_meta()
 RETURNS trigger
 LANGUAGE plpgsql
 SET search_path TO 'public', 'pg_temp'
AS $function$
...
-- FUNCTION public.archive_child_profile(p_child uuid)
CREATE OR REPLACE FUNCTION public.archive_child_profile(p_child uuid)
...
-- FUNCTION public.auth_owns_child(child uuid)
CREATE OR REPLACE FUNCTION public.auth_owns_child(child uuid)
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.bump_child_activity_summary()
CREATE OR REPLACE FUNCTION public.bump_child_activity_summary()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.bump_child_learning_state()
CREATE OR REPLACE FUNCTION public.bump_child_learning_state()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.check_join_rate_limit(in_ip_hash text)
...
-- FUNCTION public.class_assign_token(in_session_id uuid, in_class_child_id uuid, in_token text)
CREATE OR REPLACE FUNCTION public.class_assign_token(in_session_id uuid, in_class_child_id uuid, in_token text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.class_child_anonymise(in_class_child_id uuid)
...
-- FUNCTION public.class_child_archive(in_class_child_id uuid)
...
-- FUNCTION public.class_child_delete(in_class_child_id uuid)
...
-- FUNCTION public.class_delete_session(in_session_id uuid)
CREATE OR REPLACE FUNCTION public.class_delete_session(in_session_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.class_end_activity(in_session_id uuid)
...
-- FUNCTION public.class_end_session(in_session_id uuid)
...
-- FUNCTION public.class_end_stale_sessions()
CREATE OR REPLACE FUNCTION public.class_end_stale_sessions()
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.class_join_by_ip(in_session_id uuid, in_name text, in_client_ip text)
CREATE OR REPLACE FUNCTION public.class_join_by_ip(in_session_id uuid, in_name text, in_client_ip text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.class_join_with_network(in_session_id uuid, in_name text, in_network_fingerprint text)
CREATE OR REPLACE FUNCTION public.class_join_with_network(in_session_id uuid, in_name text, in_network_fingerprint text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.class_join_with_token(in_session_id uuid, in_token text)
CREATE OR REPLACE FUNCTION public.class_join_with_token(in_session_id uuid, in_token text)
 RETURNS session_students
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
-- FUNCTION public.class_kick_student(in_session_id uuid, in_student_id uuid, in_reason text)
CREATE OR REPLACE FUNCTION public.class_kick_student(in_session_id uuid, in_student_id uuid, in_reason text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.class_pause_activity(in_session_id uuid)
...
-- FUNCTION public.class_resume_activity(in_session_id uuid)
...
-- FUNCTION public.class_set_scoreboard_visibility(in_session_id uuid, in_visible boolean)
CREATE OR REPLACE FUNCTION public.class_set_scoreboard_visibility(in_session_id uuid, in_visible boolean)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.class_start_activity(in_session_id uuid, in_activity text)
CREATE OR REPLACE FUNCTION public.class_start_activity(in_session_id uuid, in_activity text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.class_student_stats(in_session_id uuid, in_student_id uuid)
CREATE OR REPLACE FUNCTION public.class_student_stats(in_session_id uuid, in_student_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.class_summary(in_session_id uuid)
CREATE OR REPLACE FUNCTION public.class_summary(in_session_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.class_validate_join(in_code text, in_network_fingerprint text)
...
-- FUNCTION public.class_validate_join_by_ip(in_code text, in_client_ip text)
CREATE OR REPLACE FUNCTION public.class_validate_join_by_ip(in_code text, in_client_ip text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.compute_billing_preview(p_interval text, p_active_children integer)
CREATE OR REPLACE FUNCTION public.compute_billing_preview(p_interval text, p_active_children integer)
 RETURNS jsonb
 LANGUAGE sql
 STABLE
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.create_child_profile(p_nickname text, p_age_band text, p_learning_focus text, p_avatar text, p_preferred_hand text, p_consent_version text)
CREATE OR REPLACE FUNCTION public.create_child_profile(p_nickname text, p_age_band text DEFAULT NULL::text, p_learning_focus text DEFAULT NULL::text, p_avatar text DEFAULT NULL::text, p_preferred_hand text DEFAULT NULL::text, p_consent_version text DEFAULT NULL::text)
 RETURNS child_profiles
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.dashboard_ab_results(in_flag text DEFAULT 'camera_explainer_v1'::text)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
CREATE OR REPLACE FUNCTION public.dashboard_adaptive_decisions(in_days integer DEFAULT 30)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.dashboard_classrooms(in_days integer DEFAULT 30)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
CREATE OR REPLACE FUNCTION public.dashboard_cohort_retention(in_weeks integer DEFAULT 8)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
CREATE OR REPLACE FUNCTION public.dashboard_context_split(in_days integer DEFAULT 30)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.dashboard_curriculum_coverage(in_days integer DEFAULT 30)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
CREATE OR REPLACE FUNCTION public.dashboard_errors(row_limit integer DEFAULT 50)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
CREATE OR REPLACE FUNCTION public.dashboard_friction_engineering(in_days integer DEFAULT 30)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.dashboard_gesture_quality(in_days integer DEFAULT 30)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.dashboard_growth(in_days integer)
...
CREATE OR REPLACE FUNCTION public.dashboard_latest_sessions(row_limit integer DEFAULT 30)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
CREATE OR REPLACE FUNCTION public.dashboard_mastery(in_days integer DEFAULT 30, in_min_attempts integer DEFAULT 5)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
CREATE OR REPLACE FUNCTION public.dashboard_mastery_milestones(in_days integer DEFAULT 60, in_min_attempts integer DEFAULT 5, in_threshold_pct integer DEFAULT 80)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
CREATE OR REPLACE FUNCTION public.dashboard_observability(in_days integer DEFAULT 7)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.dashboard_observations(in_days integer DEFAULT 30)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.dashboard_pipeline_status(in_limit integer DEFAULT 20)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.dashboard_progression_for_learner(in_device_id text)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.dashboard_progression_top_learners(in_days integer DEFAULT 30, in_limit integer DEFAULT 25)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.dashboard_tracker_health(in_days integer DEFAULT 7)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
CREATE OR REPLACE FUNCTION public.dashboard_transparency_report(in_days integer DEFAULT 90)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.dashboard_trust_strip(in_days integer DEFAULT 30)
 RETURNS jsonb
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.export_family_data()
...
-- FUNCTION public.export_parent_data()
CREATE OR REPLACE FUNCTION public.export_parent_data()
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.generate_network_fingerprint(in_ip text, in_secret text)
...
-- FUNCTION public.get_account_roles()
...
-- FUNCTION public.get_child_dashboard(p_child uuid)
CREATE OR REPLACE FUNCTION public.get_child_dashboard(p_child uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.get_effective_tier(teacher_id uuid)
CREATE OR REPLACE FUNCTION public.get_effective_tier(teacher_id uuid)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.get_email_cron_key()
CREATE OR REPLACE FUNCTION public.get_email_cron_key()
 RETURNS text
 LANGUAGE sql
 STABLE SECURITY DEFINER
...
-- FUNCTION public.get_parent_overview()
CREATE OR REPLACE FUNCTION public.get_parent_overview()
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.get_student_assignments(in_student_id uuid, in_session_id uuid)
CREATE OR REPLACE FUNCTION public.get_student_assignments(in_student_id uuid, in_session_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.get_student_class_state(in_student_id uuid, in_session_id uuid)
CREATE OR REPLACE FUNCTION public.get_student_class_state(in_student_id uuid, in_session_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.get_weekly_summary(p_child uuid)
CREATE OR REPLACE FUNCTION public.get_weekly_summary(p_child uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.handle_new_teacher_user()
...
-- FUNCTION public.has_parent_role()
CREATE OR REPLACE FUNCTION public.has_parent_role()
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.has_teacher_role()
CREATE OR REPLACE FUNCTION public.has_teacher_role()
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.initialize_teacher_trial()
CREATE OR REPLACE FUNCTION public.initialize_teacher_trial()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.is_admin_user(check_user_id uuid)
...
-- FUNCTION public.is_platform_admin()
...
-- FUNCTION public.is_school_admin(in_tenant uuid)
...
-- FUNCTION public.landing_public_proof()
...
-- FUNCTION public.learning_attempts_promote_meta()
CREATE OR REPLACE FUNCTION public.learning_attempts_promote_meta()
 RETURNS trigger
 LANGUAGE plpgsql
 SET search_path TO 'public', 'pg_temp'
AS $function$
...
-- FUNCTION public.lios_detect_anomalies_v1()
CREATE OR REPLACE FUNCTION public.lios_detect_anomalies_v1()
 RETURNS TABLE(detected bigint, by_metric jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
-- FUNCTION public.lios_detect_friction_v1(p_lookback interval)
CREATE OR REPLACE FUNCTION public.lios_detect_friction_v1(p_lookback interval DEFAULT '24:00:00'::interval)
 RETURNS TABLE(sessions_processed bigint, detectors_fired bigint, by_detector jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
-- FUNCTION public.lios_detect_mastery_episodes_v1(p_lookback interval)
CREATE OR REPLACE FUNCTION public.lios_detect_mastery_episodes_v1(p_lookback interval DEFAULT NULL::interval)
 RETURNS TABLE(pairs_processed bigint, transitions_emitted bigint, by_to_state jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.lios_recommend_next(p_device_id text, p_session_id uuid, p_game_mode text, p_current_item text, p_was_correct boolean)
CREATE OR REPLACE FUNCTION public.lios_recommend_next(p_device_id text, p_session_id uuid, p_game_mode text, p_current_item text DEFAULT NULL::text, p_was_correct boolean DEFAULT NULL::boolean)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.lios_record_observation(p_device_id text, p_focus_tags text[], p_affect_tags text[], p_independence_tags text[], p_social_tags text[], p_notable_tags text[], p_session_id uuid, p_classroom_code text, p_age_band text, p_note text, p_observer_role text)
CREATE OR REPLACE FUNCTION public.lios_record_observation(p_device_id text, p_focus_tags text[] DEFAULT '{}'::text[], p_affect_tags text[] DEFAULT '{}'::text[], p_independence_tags text[] DEFAULT '{}'::text[], p_social_tags text[] DEFAULT '{}'::text[], p_notable_tags text[] DEFAULT '{}'::text[], p_session_id uuid DEFAULT NULL::uuid, p_classroom_code text DEFAULT NULL::text, p_age_band text DEFAULT NULL::text, p_note text DEFAULT NULL::text, p_observer_role text DEFAULT 'teacher'::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.lios_run_pipeline(p_lookback interval)
CREATE OR REPLACE FUNCTION public.lios_run_pipeline(p_lookback interval DEFAULT '00:15:00'::interval)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.lios_score_unscored_attempts(p_lookback interval)
CREATE OR REPLACE FUNCTION public.lios_score_unscored_attempts(p_lookback interval DEFAULT '24:00:00'::interval)
 RETURNS TABLE(scored_rows bigint, tier_a bigint, tier_b bigint, tier_c bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
-- FUNCTION public.lios_update_elo_v1(p_lookback interval)
CREATE OR REPLACE FUNCTION public.lios_update_elo_v1(p_lookback interval DEFAULT '24:00:00'::interval)
 RETURNS TABLE(processed_attempts bigint, distinct_learners bigint, distinct_items bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
-- FUNCTION public.log_security_event(in_event_type text, in_metadata jsonb, in_ip_hash text, in_ua_hash text, in_country text)
CREATE OR REPLACE FUNCTION public.log_security_event(in_event_type text, in_metadata jsonb DEFAULT '{}'::jsonb, in_ip_hash text DEFAULT NULL::text, in_ua_hash text DEFAULT NULL::text, in_country text DEFAULT NULL::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.normalize_ip_for_network(in_ip text)
...
-- FUNCTION public.parent_has_access(p_parent uuid)
CREATE OR REPLACE FUNCTION public.parent_has_access(p_parent uuid)
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.parent_subscription_state(p_parent uuid)
...
-- FUNCTION public.pricing_amount_cents(p_interval text, p_active_children integer)
CREATE OR REPLACE FUNCTION public.pricing_amount_cents(p_interval text, p_active_children integer)
 RETURNS integer
 LANGUAGE sql
 STABLE
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.process_account_deletion_requests(p_limit integer)
CREATE OR REPLACE FUNCTION public.process_account_deletion_requests(p_limit integer DEFAULT 50)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.record_consent(p_consent_type text, p_consent_version text, p_granted boolean)
CREATE OR REPLACE FUNCTION public.record_consent(p_consent_type text, p_consent_version text, p_granted boolean DEFAULT true)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.register_teacher(in_full_name text, in_school_name text)
CREATE OR REPLACE FUNCTION public.register_teacher(in_full_name text DEFAULT NULL::text, in_school_name text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.request_account_deletion()
...
-- FUNCTION public.request_child_deletion(p_child uuid)
CREATE OR REPLACE FUNCTION public.request_child_deletion(p_child uuid)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.restore_child_profile(p_child uuid)
CREATE OR REPLACE FUNCTION public.restore_child_profile(p_child uuid)
 RETURNS child_profiles
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.session_lookup_by_code(in_code text)
...
-- FUNCTION public.session_set_network_fingerprint(in_session_id uuid, in_teacher_ip text)
CREATE OR REPLACE FUNCTION public.session_set_network_fingerprint(in_session_id uuid, in_teacher_ip text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.session_validate_network(in_session_id uuid, in_network_fingerprint text)
CREATE OR REPLACE FUNCTION public.session_validate_network(in_session_id uuid, in_network_fingerprint text)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.set_classroom_default_activities(in_session_id uuid, in_activities text[])
CREATE OR REPLACE FUNCTION public.set_classroom_default_activities(in_session_id uuid, in_activities text[])
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.set_session_school_id()
CREATE OR REPLACE FUNCTION public.set_session_school_id()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.set_student_assignments(in_session_id uuid, in_student_id uuid, in_activities jsonb)
CREATE OR REPLACE FUNCTION public.set_student_assignments(in_session_id uuid, in_student_id uuid, in_activities jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
-- FUNCTION public.start_parent_trial(p_parent uuid)
...
-- FUNCTION public.sync_teacher_school_tier()
CREATE OR REPLACE FUNCTION public.sync_teacher_school_tier()
 RETURNS trigger
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- FUNCTION public.touch_class_children_updated_at()
CREATE OR REPLACE FUNCTION public.touch_class_children_updated_at()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$
...
-- FUNCTION public.user_tenant_ids()
...
CREATE OR REPLACE VIEW public.v_engagement_metrics WITH (security_invoker=true) AS
 SELECT to_char(date_trunc('week'::text, s.created_at), 'YYYY-MM-DD'::text) AS period,
   ...
   FROM sessions s
     LEFT JOIN session_students ss ON ss.session_id = s.id
     LEFT JOIN round_scores rs ON rs.session_id = s.id
...
CREATE OR REPLACE VIEW public.v_school_overview WITH (security_invoker=true) AS
 SELECT sc.id AS school_id,
   ...
   FROM schools sc
     LEFT JOIN school_teachers st ON st.school_id = sc.id
     LEFT JOIN sessions s ON s.school_id = sc.id
...
CREATE OR REPLACE VIEW public.v_growth_metrics WITH (security_invoker=true) AS
 SELECT to_char(date_trunc('day'::text, t.created_at), 'YYYY-MM-DD'::text) AS cohort_date,
    ...
    0 AS churn_count
   FROM teachers t
     LEFT JOIN schools sc ON date_trunc('day'::text, sc.created_at) = date_trunc('day'::text, t.created_at)
...
-- View comments, copied verbatim from the production catalog (obj_description) on 2026-09-08.
...
-- ---------- constraints (pk, unique, check, fk; constraint triggers are in the triggers section) ----------
...
ALTER TABLE public.class_session_tokens ADD CONSTRAINT class_session_tokens_pkey PRIMARY KEY (id);
...
ALTER TABLE public.class_session_tokens ADD CONSTRAINT class_session_tokens_session_id_class_child_id_key UNIQUE (session_id, class_child_id);
ALTER TABLE public.class_session_tokens ADD CONSTRAINT class_session_tokens_session_id_token_key UNIQUE (session_id, token);
...
ALTER TABLE public.session_students ADD CONSTRAINT session_students_readiness_state_check CHECK ((readiness_state = ANY (ARRAY['joined'::text, 'camera_permission_needed'::text, 'camera_ready'::text, 'hand_detected'::text, 'ready'::text, 'playing'::text, 'tracking_lost'::text, 'needs_help'::text, 'completed'::text, 'disconnected'::text, 'removed'::text])));
...
ALTER TABLE public.class_session_tokens ADD CONSTRAINT class_session_tokens_claimed_session_student_id_fkey FOREIGN KEY (claimed_session_student_id) REFERENCES session_students(id) ON DELETE SET NULL;
ALTER TABLE public.class_session_tokens ADD CONSTRAINT class_session_tokens_class_child_id_fkey FOREIGN KEY (class_child_id) REFERENCES class_children(id) ON DELETE CASCADE;
ALTER TABLE public.class_session_tokens ADD CONSTRAINT class_session_tokens_session_id_fkey FOREIGN KEY (session_id) REFERENCES sessions(id) ON DELETE CASCADE;
ALTER TABLE public.class_session_tokens ADD CONSTRAINT class_session_tokens_tenant_id_fkey FOREIGN KEY (tenant_id) REFERENCES tenants(id);
...
CREATE INDEX IF NOT EXISTS analytics_events_attempt_idx ON public.analytics_events USING btree (attempt_id, occurred_at) WHERE (attempt_id IS NOT NULL);
CREATE INDEX IF NOT EXISTS analytics_events_device_idx ON public.analytics_events USING btree (device_id, occurred_at DESC) WHERE (device_id IS NOT NULL);
CREATE UNIQUE INDEX analytics_events_event_uid_uidx ON public.analytics_events USING btree (event_uid);
CREATE INDEX IF NOT EXISTS analytics_events_mode_idx ON public.analytics_events USING btree (game_mode, occurred_at DESC) WHERE (game_mode IS NOT NULL);
CREATE INDEX IF NOT EXISTS analytics_events_name_idx ON public.analytics_events USING btree (event_name, occurred_at DESC);
CREATE INDEX IF NOT EXISTS analytics_events_occurred_at_idx ON public.analytics_events USING btree (occurred_at DESC);
CREATE INDEX IF NOT EXISTS analytics_events_real_prod_idx ON public.analytics_events USING btree (occurred_at DESC) WHERE ((traffic_type = 'real'::text) AND (environment = 'production'::text));
CREATE INDEX IF NOT EXISTS analytics_events_session_idx ON public.analytics_events USING btree (session_id, occurred_at);
CREATE INDEX IF NOT EXISTS idx_billing_events_parent ON public.billing_events USING btree (parent_id);
CREATE INDEX IF NOT EXISTS idx_activity_summary_child ON public.child_activity_summary USING btree (child_profile_id);
CREATE INDEX IF NOT EXISTS idx_child_profiles_parent ON public.child_profiles USING btree (parent_id);
CREATE INDEX IF NOT EXISTS idx_child_profiles_status ON public.child_profiles USING btree (parent_id, status);
CREATE INDEX IF NOT EXISTS idx_child_profiles_tenant ON public.child_profiles USING btree (tenant_id) WHERE (tenant_id IS NOT NULL);
CREATE INDEX IF NOT EXISTS class_children_teacher_idx ON public.class_children USING btree (teacher_id) WHERE (archived = false);
CREATE INDEX IF NOT EXISTS idx_client_errors_reported ON public.client_errors USING btree (reported_at DESC);
CREATE INDEX IF NOT EXISTS idx_client_errors_session_id ON public.client_errors USING btree (session_id);
CREATE INDEX IF NOT EXISTS idx_consent_parent ON public.consent_records USING btree (parent_id);
CREATE INDEX IF NOT EXISTS data_deletion_requests_target_child_id_idx ON public.data_deletion_requests USING btree (target_child_id);
CREATE INDEX IF NOT EXISTS idx_deletion_parent ON public.data_deletion_requests USING btree (parent_id);
CREATE INDEX IF NOT EXISTS idx_form_submissions_created ON public.form_submissions USING btree (created_at DESC);
CREATE INDEX IF NOT EXISTS human_observation_recorded_idx ON public.human_observation_fact USING btree (recorded_at DESC);
CREATE INDEX IF NOT EXISTS idx_attempts_child ON public.learning_attempts USING btree (child_profile_id);
CREATE INDEX IF NOT EXISTS idx_learning_attempts_tenant ON public.learning_attempts USING btree (tenant_id) WHERE (tenant_id IS NOT NULL);
CREATE INDEX IF NOT EXISTS learning_attempts_device_item_idx ON public.learning_attempts USING btree (device_id, item_key, occurred_at DESC);
CREATE INDEX IF NOT EXISTS learning_attempts_elo_pending_idx ON public.learning_attempts USING btree (occurred_at) WHERE ((elo_processed_at IS NULL) AND (credibility_score IS NOT NULL));
CREATE UNIQUE INDEX learning_attempts_event_uid_uidx ON public.learning_attempts USING btree (event_uid);
CREATE INDEX IF NOT EXISTS learning_attempts_mode_idx ON public.learning_attempts USING btree (game_mode, occurred_at DESC);
CREATE INDEX IF NOT EXISTS learning_attempts_session_idx ON public.learning_attempts USING btree (session_id, occurred_at);
CREATE INDEX IF NOT EXISTS learning_attempts_tier_idx ON public.learning_attempts USING btree (credibility_tier, occurred_at DESC) WHERE (credibility_tier IS NOT NULL);
CREATE INDEX IF NOT EXISTS learning_attempts_unscored_idx ON public.learning_attempts USING btree (occurred_at DESC) WHERE (credibility_score IS NULL);
CREATE INDEX IF NOT EXISTS lios_adaptive_decisions_device_idx ON public.lios_adaptive_decisions USING btree (device_id, made_at DESC);
CREATE INDEX IF NOT EXISTS lios_adaptive_decisions_made_idx ON public.lios_adaptive_decisions USING btree (made_at DESC);
CREATE INDEX IF NOT EXISTS lios_anomaly_detected_idx ON public.lios_anomaly_fact USING btree (detected_at DESC);
CREATE INDEX IF NOT EXISTS lios_anomaly_metric_idx ON public.lios_anomaly_fact USING btree (metric, detected_at DESC);
CREATE INDEX IF NOT EXISTS lios_pipeline_runs_time_idx ON public.lios_pipeline_runs USING btree (run_at DESC);
CREATE INDEX IF NOT EXISTS mastery_episode_learner_idx ON public.mastery_episode_fact USING btree (device_id, item_key, game_mode, transition_at DESC);
CREATE INDEX IF NOT EXISTS mastery_episode_state_idx ON public.mastery_episode_fact USING btree (to_state, transition_at DESC);
CREATE INDEX IF NOT EXISTS mastery_episode_time_idx ON public.mastery_episode_fact USING btree (transition_at DESC);
CREATE INDEX IF NOT EXISTS idx_parent_controls_parent ON public.parent_controls USING btree (parent_id);
CREATE INDEX IF NOT EXISTS idx_parent_subscriptions_customer ON public.parent_subscriptions USING btree (stripe_customer_id);
CREATE INDEX IF NOT EXISTS idx_parent_subscriptions_parent ON public.parent_subscriptions USING btree (parent_id);
CREATE INDEX IF NOT EXISTS idx_playlists_school_id ON public.playlists USING btree (school_id);
CREATE INDEX IF NOT EXISTS idx_playlists_teacher ON public.playlists USING btree (teacher_id);
CREATE INDEX IF NOT EXISTS idx_round_scores_activity ON public.round_scores USING btree (activity);
CREATE INDEX IF NOT EXISTS idx_round_scores_session_id ON public.round_scores USING btree (session_id);
...
CREATE INDEX IF NOT EXISTS round_scores_activity_idx ON public.round_scores USING btree (session_activity_id);
CREATE INDEX IF NOT EXISTS round_scores_session_idx ON public.round_scores USING btree (session_id, round);
CREATE INDEX IF NOT EXISTS round_scores_student_idx ON public.round_scores USING btree (student_id);
CREATE INDEX IF NOT EXISTS idx_school_invites_invited_by ON public.school_invites USING btree (invited_by);
CREATE INDEX IF NOT EXISTS idx_school_invites_school_id ON public.school_invites USING btree (school_id);
CREATE INDEX IF NOT EXISTS idx_school_teachers_invited_by ON public.school_teachers USING btree (invited_by);
CREATE INDEX IF NOT EXISTS idx_school_teachers_school ON public.school_teachers USING btree (school_id);
CREATE INDEX IF NOT EXISTS idx_school_teachers_teacher ON public.school_teachers USING btree (teacher_id);
CREATE INDEX IF NOT EXISTS idx_schools_admin ON public.schools USING btree (admin_user_id);
CREATE INDEX IF NOT EXISTS idx_sec_audit_created ON public.security_audit_log USING btree (created_at DESC);
CREATE INDEX IF NOT EXISTS idx_sec_audit_type ON public.security_audit_log USING btree (event_type, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_sec_audit_user ON public.security_audit_log USING btree (user_id, created_at DESC);
CREATE INDEX IF NOT EXISTS session_activities_session_idx ON public.session_activities USING btree (session_id, started_at DESC);
CREATE INDEX IF NOT EXISTS idx_network_fingerprints_hash ON public.session_network_fingerprints USING btree (network_hash);
CREATE INDEX IF NOT EXISTS idx_network_fingerprints_session ON public.session_network_fingerprints USING btree (session_id);
CREATE INDEX IF NOT EXISTS idx_session_students_tenant ON public.session_students USING btree (tenant_id) WHERE (tenant_id IS NOT NULL);
CREATE INDEX IF NOT EXISTS session_students_active_idx ON public.session_students USING btree (session_id) WHERE (kicked_at IS NULL);
CREATE INDEX IF NOT EXISTS session_students_class_child_idx ON public.session_students USING btree (class_child_id) WHERE (class_child_id IS NOT NULL);
CREATE INDEX IF NOT EXISTS session_students_readiness_idx ON public.session_students USING btree (session_id, readiness_state);
CREATE INDEX IF NOT EXISTS session_students_session_idx ON public.session_students USING btree (session_id);
CREATE INDEX IF NOT EXISTS idx_sessions_school_id ON public.sessions USING btree (school_id);
CREATE INDEX IF NOT EXISTS idx_sessions_teacher_id ON public.sessions USING btree (teacher_id);
CREATE INDEX IF NOT EXISTS idx_sessions_teacher_status ON public.sessions USING btree (teacher_id, status);
CREATE INDEX IF NOT EXISTS idx_sessions_tenant ON public.sessions USING btree (tenant_id) WHERE (tenant_id IS NOT NULL);
CREATE UNIQUE INDEX sessions_active_code_idx ON public.sessions USING btree (code) WHERE (status <> 'ended'::text);
CREATE INDEX IF NOT EXISTS sessions_current_activity_id_idx ON public.sessions USING btree (current_activity_id);
CREATE INDEX IF NOT EXISTS sessions_playlist_id_idx ON public.sessions USING btree (playlist_id);
CREATE INDEX IF NOT EXISTS sessions_teacher_idx ON public.sessions USING btree (teacher_id, created_at DESC);
CREATE INDEX IF NOT EXISTS skill_state_device_idx ON public.skill_state USING btree (device_id, last_attempt_at DESC);
CREATE INDEX IF NOT EXISTS skill_state_item_idx ON public.skill_state USING btree (item_key, game_mode, theta DESC);
CREATE INDEX IF NOT EXISTS idx_student_assignments_session_activity ON public.student_activity_assignments USING btree (session_id, activity);
CREATE INDEX IF NOT EXISTS idx_student_assignments_session_student_seq ON public.student_activity_assignments USING btree (session_id, student_id, sequence_order);
CREATE INDEX IF NOT EXISTS idx_student_assignments_student ON public.student_activity_assignments USING btree (student_id);
CREATE INDEX IF NOT EXISTS idx_teacher_insights_expires ON public.teacher_insights USING btree (expires_at);
CREATE INDEX IF NOT EXISTS idx_teacher_insights_teacher ON public.teacher_insights USING btree (teacher_id);
CREATE INDEX IF NOT EXISTS idx_teachers_school_id ON public.teachers USING btree (school_id);

-- ---------- triggers ----------
CREATE TRIGGER trg_analytics_events_promote_meta BEFORE INSERT ON public.analytics_events FOR EACH ROW EXECUTE FUNCTION analytics_events_promote_meta();
CREATE TRIGGER stamp_tenant BEFORE INSERT ON public.billing_events FOR EACH ROW EXECUTE FUNCTION _stamp_parent_tenant();
CREATE TRIGGER stamp_tenant BEFORE INSERT ON public.child_activity_summary FOR EACH ROW EXECUTE FUNCTION _stamp_child_tenant();
CREATE TRIGGER trg_child_activity_summary_updated BEFORE UPDATE ON public.child_activity_summary FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER stamp_tenant BEFORE INSERT ON public.child_learning_state FOR EACH ROW EXECUTE FUNCTION _stamp_child_tenant();
CREATE TRIGGER trg_child_learning_state_updated BEFORE UPDATE ON public.child_learning_state FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER stamp_tenant BEFORE INSERT ON public.child_profiles FOR EACH ROW EXECUTE FUNCTION _stamp_parent_tenant();
CREATE TRIGGER trg_child_profiles_updated BEFORE UPDATE ON public.child_profiles FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER trg_class_children_touch BEFORE UPDATE ON public.class_children FOR EACH ROW EXECUTE FUNCTION touch_class_children_updated_at();
CREATE TRIGGER stamp_tenant BEFORE INSERT ON public.consent_records FOR EACH ROW EXECUTE FUNCTION _stamp_parent_tenant();
CREATE TRIGGER stamp_tenant BEFORE INSERT ON public.data_deletion_requests FOR EACH ROW EXECUTE FUNCTION _stamp_parent_tenant();
CREATE TRIGGER stamp_tenant BEFORE INSERT ON public.learning_attempts FOR EACH ROW EXECUTE FUNCTION _stamp_child_tenant();
CREATE TRIGGER trg_attempt_bumps_activity_summary AFTER INSERT ON public.learning_attempts FOR EACH ROW EXECUTE FUNCTION bump_child_activity_summary();
CREATE TRIGGER trg_attempt_bumps_learning_state AFTER INSERT ON public.learning_attempts FOR EACH ROW EXECUTE FUNCTION bump_child_learning_state();
CREATE TRIGGER trg_learning_attempts_promote_meta BEFORE INSERT ON public.learning_attempts FOR EACH ROW EXECUTE FUNCTION learning_attempts_promote_meta();
CREATE TRIGGER stamp_tenant BEFORE INSERT ON public.parent_controls FOR EACH ROW EXECUTE FUNCTION _stamp_parent_tenant();
CREATE TRIGGER trg_parent_controls_updated BEFORE UPDATE ON public.parent_controls FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER stamp_tenant BEFORE INSERT ON public.parent_profiles FOR EACH ROW EXECUTE FUNCTION _stamp_parent_profile_tenant();
CREATE TRIGGER trg_parent_profiles_updated BEFORE UPDATE ON public.parent_profiles FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER stamp_tenant BEFORE INSERT ON public.parent_subscriptions FOR EACH ROW EXECUTE FUNCTION _stamp_parent_tenant();
CREATE TRIGGER trg_parent_subscriptions_updated BEFORE UPDATE ON public.parent_subscriptions FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER playlists_updated_at BEFORE UPDATE ON public.playlists FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER stamp_tenant BEFORE INSERT ON public.playlists FOR EACH ROW EXECUTE FUNCTION _stamp_teacher_tenant();
...
CREATE TRIGGER stamp_tenant BEFORE INSERT ON public.round_scores FOR EACH ROW EXECUTE FUNCTION _stamp_session_tenant();
CREATE TRIGGER trg_sync_teacher_school_tier AFTER INSERT OR UPDATE ON public.school_teachers FOR EACH ROW EXECUTE FUNCTION sync_teacher_school_tier();
CREATE TRIGGER schools_updated_at BEFORE UPDATE ON public.schools FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER stamp_tenant BEFORE INSERT ON public.session_activities FOR EACH ROW EXECUTE FUNCTION _stamp_session_tenant();
CREATE TRIGGER stamp_tenant BEFORE INSERT ON public.session_students FOR EACH ROW EXECUTE FUNCTION _stamp_session_tenant();
CREATE TRIGGER sessions_updated_at BEFORE UPDATE ON public.sessions FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER stamp_tenant BEFORE INSERT ON public.sessions FOR EACH ROW EXECUTE FUNCTION _stamp_teacher_tenant();
CREATE CONSTRAINT TRIGGER trg_sessions_check_active_has_activity AFTER INSERT OR UPDATE OF class_state, current_activity_id ON public.sessions DEFERRABLE INITIALLY DEFERRED FOR EACH ROW EXECUTE FUNCTION _check_active_has_activity();
CREATE TRIGGER trg_set_session_school_id BEFORE INSERT ON public.sessions FOR EACH ROW EXECUTE FUNCTION set_session_school_id();
CREATE TRIGGER trg_stripe_price_map_updated BEFORE UPDATE ON public.stripe_price_map FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER stamp_tenant BEFORE INSERT ON public.teacher_insights FOR EACH ROW EXECUTE FUNCTION _stamp_teacher_tenant();
CREATE TRIGGER teachers_updated_at BEFORE UPDATE ON public.teachers FOR EACH ROW EXECUTE FUNCTION set_updated_at();
CREATE TRIGGER trg_initialize_trial BEFORE INSERT ON public.teachers FOR EACH ROW EXECUTE FUNCTION initialize_teacher_trial();

-- ---------- app-owned triggers on auth.users (signup handlers) ----------
...
CREATE POLICY admin_alerts_select ON public.admin_alerts AS PERMISSIVE FOR SELECT TO public USING (is_admin_user(( SELECT auth.uid() AS uid)));
CREATE POLICY admin_alerts_update ON public.admin_alerts AS PERMISSIVE FOR UPDATE TO public USING (is_admin_user(( SELECT auth.uid() AS uid)));
CREATE POLICY "Anonymous insert analytics events" ON public.analytics_events AS PERMISSIVE FOR INSERT TO anon, authenticated WITH CHECK (true);
CREATE POLICY analytics_events_select_admin ON public.analytics_events AS PERMISSIVE FOR SELECT TO authenticated USING (is_admin_user(( SELECT auth.uid() AS uid)));
CREATE POLICY billing_events_select ON public.billing_events AS PERMISSIVE FOR SELECT TO authenticated USING ((parent_id = auth.uid()));
CREATE POLICY cas_delete ON public.child_activity_summary AS PERMISSIVE FOR DELETE TO authenticated USING (auth_owns_child(child_profile_id));
CREATE POLICY cas_insert ON public.child_activity_summary AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (auth_owns_child(child_profile_id));
CREATE POLICY cas_select ON public.child_activity_summary AS PERMISSIVE FOR SELECT TO authenticated USING (auth_owns_child(child_profile_id));
CREATE POLICY cas_update ON public.child_activity_summary AS PERMISSIVE FOR UPDATE TO authenticated USING (auth_owns_child(child_profile_id)) WITH CHECK (auth_owns_child(child_profile_id));
CREATE POLICY cas_write ON public.child_activity_summary AS PERMISSIVE FOR ALL TO authenticated USING (auth_owns_child(child_profile_id)) WITH CHECK (auth_owns_child(child_profile_id));
CREATE POLICY cls_delete ON public.child_learning_state AS PERMISSIVE FOR DELETE TO authenticated USING (auth_owns_child(child_profile_id));
CREATE POLICY cls_insert ON public.child_learning_state AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (auth_owns_child(child_profile_id));
CREATE POLICY cls_select ON public.child_learning_state AS PERMISSIVE FOR SELECT TO authenticated USING (auth_owns_child(child_profile_id));
CREATE POLICY cls_update ON public.child_learning_state AS PERMISSIVE FOR UPDATE TO authenticated USING (auth_owns_child(child_profile_id)) WITH CHECK (auth_owns_child(child_profile_id));
CREATE POLICY cls_write ON public.child_learning_state AS PERMISSIVE FOR ALL TO authenticated USING (auth_owns_child(child_profile_id)) WITH CHECK (auth_owns_child(child_profile_id));
CREATE POLICY child_profiles_delete ON public.child_profiles AS PERMISSIVE FOR DELETE TO authenticated USING ((parent_id = auth.uid()));
CREATE POLICY child_profiles_insert ON public.child_profiles AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((parent_id = auth.uid()));
CREATE POLICY child_profiles_select ON public.child_profiles AS PERMISSIVE FOR SELECT TO authenticated USING ((parent_id = auth.uid()));
CREATE POLICY child_profiles_update ON public.child_profiles AS PERMISSIVE FOR UPDATE TO authenticated USING ((parent_id = auth.uid())) WITH CHECK ((parent_id = auth.uid()));
CREATE POLICY class_children_all_own ON public.class_children AS PERMISSIVE FOR ALL TO authenticated USING ((teacher_id = ( SELECT auth.uid() AS uid))) WITH CHECK ((teacher_id = ( SELECT auth.uid() AS uid)));
CREATE POLICY cst_teacher_rw ON public.class_session_tokens AS PERMISSIVE FOR ALL TO authenticated USING ((EXISTS ( SELECT 1
   FROM sessions s
  WHERE ((s.id = class_session_tokens.session_id) AND (s.teacher_id = auth.uid()))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM sessions s
  WHERE ((s.id = class_session_tokens.session_id) AND (s.teacher_id = auth.uid())))));
CREATE POLICY "Teacher manages classroom defaults" ON public.classroom_default_activities AS PERMISSIVE FOR ALL TO authenticated USING ((EXISTS ( SELECT 1
   FROM sessions s
...
CREATE POLICY client_errors_insert ON public.client_errors AS PERMISSIVE FOR INSERT TO public WITH CHECK (true);
CREATE POLICY client_errors_select_admin ON public.client_errors AS PERMISSIVE FOR SELECT TO public USING (is_admin_user(( SELECT auth.uid() AS uid)));
CREATE POLICY consent_insert ON public.consent_records AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((parent_id = auth.uid()));
CREATE POLICY consent_select ON public.consent_records AS PERMISSIVE FOR SELECT TO authenticated USING ((parent_id = auth.uid()));
CREATE POLICY consent_update ON public.consent_records AS PERMISSIVE FOR UPDATE TO authenticated USING ((parent_id = auth.uid())) WITH CHECK ((parent_id = auth.uid()));
CREATE POLICY deletion_insert ON public.data_deletion_requests AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((parent_id = auth.uid()));
CREATE POLICY deletion_select ON public.data_deletion_requests AS PERMISSIVE FOR SELECT TO authenticated USING ((parent_id = auth.uid()));
CREATE POLICY "Service role can insert form submissions" ON public.form_submissions AS PERMISSIVE FOR INSERT TO public WITH CHECK (true);
CREATE POLICY admin_read_form_submissions ON public.form_submissions AS PERMISSIVE FOR SELECT TO public USING (is_admin_user(( SELECT auth.uid() AS uid)));
CREATE POLICY "Authenticated insert observations" ON public.human_observation_fact AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (true);
CREATE POLICY observations_select_admin ON public.human_observation_fact AS PERMISSIVE FOR SELECT TO authenticated USING (is_admin_user(( SELECT auth.uid() AS uid)));
CREATE POLICY "Authenticated select item_difficulty" ON public.item_difficulty AS PERMISSIVE FOR SELECT TO authenticated USING (true);
CREATE POLICY "Teachers read join audit log for own sessions" ON public.join_audit_log AS PERMISSIVE FOR SELECT TO authenticated USING (((EXISTS ( SELECT 1
   FROM sessions s
  WHERE ((s.id = join_audit_log.session_id) AND (s.teacher_id = auth.uid())))) OR _is_admin()));
CREATE POLICY "service_role write join audit log" ON public.join_audit_log AS PERMISSIVE FOR INSERT TO service_role WITH CHECK (true);
CREATE POLICY attempts_child_rows ON public.learning_attempts AS PERMISSIVE FOR ALL TO authenticated USING (((child_profile_id IS NOT NULL) AND auth_owns_child(child_profile_id))) WITH CHECK (((child_profile_id IS NOT NULL) AND auth_owns_child(child_profile_id)));
CREATE POLICY attempts_delete ON public.learning_attempts AS PERMISSIVE FOR DELETE TO anon, authenticated USING (((child_profile_id IS NULL) OR ((child_profile_id IS NOT NULL) AND auth_owns_child(child_profile_id))));
CREATE POLICY attempts_insert ON public.learning_attempts AS PERMISSIVE FOR INSERT TO anon, authenticated WITH CHECK (((child_profile_id IS NULL) OR ((child_profile_id IS NOT NULL) AND auth_owns_child(child_profile_id))));
CREATE POLICY attempts_school_rows ON public.learning_attempts AS PERMISSIVE FOR ALL TO anon, authenticated USING ((child_profile_id IS NULL)) WITH CHECK ((child_profile_id IS NULL));
CREATE POLICY attempts_select_scoped ON public.learning_attempts AS PERMISSIVE FOR SELECT TO authenticated USING ((is_admin_user(( SELECT auth.uid() AS uid)) OR ((child_profile_id IS NOT NULL) AND auth_owns_child(child_profile_id))));
CREATE POLICY attempts_update ON public.learning_attempts AS PERMISSIVE FOR UPDATE TO anon, authenticated USING (((child_profile_id IS NULL) OR ((child_profile_id IS NOT NULL) AND auth_owns_child(child_profile_id)))) WITH CHECK (((child_profile_id IS NULL) OR ((child_profile_id IS NOT NULL) AND auth_owns_child(child_profile_id))));
CREATE POLICY adaptive_select_admin ON public.lios_adaptive_decisions AS PERMISSIVE FOR SELECT TO authenticated USING (is_admin_user(( SELECT auth.uid() AS uid)));
CREATE POLICY anomaly_select_admin ON public.lios_anomaly_fact AS PERMISSIVE FOR SELECT TO authenticated USING (is_admin_user(( SELECT auth.uid() AS uid)));
CREATE POLICY lios_runs_select_admin ON public.lios_pipeline_runs AS PERMISSIVE FOR SELECT TO authenticated USING (is_admin_user(( SELECT auth.uid() AS uid)));
CREATE POLICY mastery_select_admin ON public.mastery_episode_fact AS PERMISSIVE FOR SELECT TO authenticated USING (is_admin_user(( SELECT auth.uid() AS uid)));
CREATE POLICY "Service role can insert subscribers" ON public.newsletter_subscribers AS PERMISSIVE FOR INSERT TO public WITH CHECK (true);
CREATE POLICY admin_read_newsletter_subscribers ON public.newsletter_subscribers AS PERMISSIVE FOR SELECT TO public USING (is_admin_user(( SELECT auth.uid() AS uid)));
CREATE POLICY controls_delete ON public.parent_controls AS PERMISSIVE FOR DELETE TO authenticated USING ((parent_id = ( SELECT auth.uid() AS uid)));
CREATE POLICY controls_insert ON public.parent_controls AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (((parent_id = ( SELECT auth.uid() AS uid)) AND auth_owns_child(child_profile_id)));
CREATE POLICY controls_select ON public.parent_controls AS PERMISSIVE FOR SELECT TO authenticated USING ((parent_id = auth.uid()));
CREATE POLICY controls_update ON public.parent_controls AS PERMISSIVE FOR UPDATE TO authenticated USING ((parent_id = ( SELECT auth.uid() AS uid))) WITH CHECK (((parent_id = ( SELECT auth.uid() AS uid)) AND auth_owns_child(child_profile_id)));
CREATE POLICY controls_write ON public.parent_controls AS PERMISSIVE FOR ALL TO authenticated USING ((parent_id = auth.uid())) WITH CHECK (((parent_id = auth.uid()) AND auth_owns_child(child_profile_id)));
CREATE POLICY parent_profiles_insert ON public.parent_profiles AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((id = auth.uid()));
CREATE POLICY parent_profiles_select ON public.parent_profiles AS PERMISSIVE FOR SELECT TO authenticated USING ((id = auth.uid()));
CREATE POLICY parent_profiles_update ON public.parent_profiles AS PERMISSIVE FOR UPDATE TO authenticated USING ((id = auth.uid())) WITH CHECK ((id = auth.uid()));
CREATE POLICY parent_subscriptions_select ON public.parent_subscriptions AS PERMISSIVE FOR SELECT TO authenticated USING ((parent_id = auth.uid()));
CREATE POLICY platform_admins_no_client ON public.platform_admins AS PERMISSIVE FOR ALL TO authenticated USING (false) WITH CHECK (false);
CREATE POLICY platform_insights_admin ON public.platform_insights AS PERMISSIVE FOR SELECT TO public USING (COALESCE(( SELECT teachers.is_admin
   FROM teachers
  WHERE (teachers.id = ( SELECT auth.uid() AS uid))), false));
CREATE POLICY playlists_delete_own ON public.playlists AS PERMISSIVE FOR DELETE TO public USING ((teacher_id = ( SELECT auth.uid() AS uid)));
CREATE POLICY playlists_insert_pro ON public.playlists AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK ((get_effective_tier(( SELECT auth.uid() AS uid)) = ANY (ARRAY['pro'::text, 'admin'::text])));
CREATE POLICY playlists_select ON public.playlists AS PERMISSIVE FOR SELECT TO public USING (((teacher_id = ( SELECT auth.uid() AS uid)) OR (is_public = true) OR COALESCE(( SELECT teachers.is_admin
   FROM teachers
  WHERE (teachers.id = ( SELECT auth.uid() AS uid))), false)));
CREATE POLICY playlists_update_own ON public.playlists AS PERMISSIVE FOR UPDATE TO public USING ((teacher_id = ( SELECT auth.uid() AS uid)));
CREATE POLICY pricing_config_read ON public.pricing_config AS PERMISSIVE FOR SELECT TO anon, authenticated USING (true);
CREATE POLICY round_scores_select_scoped ON public.round_scores AS PERMISSIVE FOR SELECT TO public USING (((EXISTS ( SELECT 1
   FROM sessions s
  WHERE ((s.id = round_scores.session_id) AND (s.teacher_id = ( SELECT auth.uid() AS uid))))) OR is_admin_user(( SELECT auth.uid() AS uid)) OR (EXISTS ( SELECT 1
   FROM sessions s
  WHERE ((s.id = round_scores.session_id) AND (s.status <> 'ended'::text))))));
CREATE POLICY submit_score_active_session_only ON public.round_scores AS PERMISSIVE FOR INSERT TO public WITH CHECK ((EXISTS ( SELECT 1
   FROM sessions s
  WHERE ((s.id = round_scores.session_id) AND (s.status = ANY (ARRAY['active'::text, 'playing'::text]))))));
CREATE POLICY invites_insert_admin ON public.school_invites AS PERMISSIVE FOR INSERT TO public WITH CHECK ((school_id IN ( SELECT schools.id
   FROM schools
  WHERE (schools.admin_user_id = ( SELECT auth.uid() AS uid)))));
CREATE POLICY invites_select ON public.school_invites AS PERMISSIVE FOR SELECT TO public USING (((school_id IN ( SELECT schools.id
   FROM schools
  WHERE (schools.admin_user_id = ( SELECT auth.uid() AS uid)))) OR COALESCE(( SELECT teachers.is_admin
   FROM teachers
  WHERE (teachers.id = ( SELECT auth.uid() AS uid))), false)));
CREATE POLICY school_teachers_insert_admin ON public.school_teachers AS PERMISSIVE FOR INSERT TO public WITH CHECK ((school_id IN ( SELECT schools.id
   FROM schools
  WHERE (schools.admin_user_id = ( SELECT auth.uid() AS uid)))));
CREATE POLICY school_teachers_select ON public.school_teachers AS PERMISSIVE FOR SELECT TO public USING (((school_id IN ( SELECT schools.id
   FROM schools
  WHERE (schools.admin_user_id = ( SELECT auth.uid() AS uid)))) OR (teacher_id = ( SELECT auth.uid() AS uid)) OR COALESCE(( SELECT teachers.is_admin
   FROM teachers
  WHERE (teachers.id = ( SELECT auth.uid() AS uid))), false)));
CREATE POLICY school_teachers_update_admin ON public.school_teachers AS PERMISSIVE FOR UPDATE TO public USING ((school_id IN ( SELECT schools.id
   FROM schools
  WHERE (schools.admin_user_id = ( SELECT auth.uid() AS uid)))));
CREATE POLICY schools_insert ON public.schools AS PERMISSIVE FOR INSERT TO public WITH CHECK ((admin_user_id = ( SELECT auth.uid() AS uid)));
CREATE POLICY schools_select_own ON public.schools AS PERMISSIVE FOR SELECT TO public USING (((admin_user_id = ( SELECT auth.uid() AS uid)) OR (id IN ( SELECT school_teachers.school_id
  ...
  WHERE ((school_teachers.teacher_id = ( SELECT auth.uid() AS uid)) AND (school_teachers.status = 'active'::text)))) OR COALESCE(( SELECT teachers.is_admin
   FROM teachers
  WHERE (teachers.id = ( SELECT auth.uid() AS uid))), false)));
CREATE POLICY schools_update_admin ON public.schools AS PERMISSIVE FOR UPDATE TO public USING ((admin_user_id = ( SELECT auth.uid() AS uid)));
CREATE POLICY sec_audit_select_admin ON public.security_audit_log AS PERMISSIVE FOR SELECT TO authenticated USING (is_admin_user(( SELECT auth.uid() AS uid)));
CREATE POLICY sec_audit_select_own ON public.security_audit_log AS PERMISSIVE FOR SELECT TO authenticated USING ((user_id = ( SELECT auth.uid() AS uid)));
CREATE POLICY "Read session activities by session" ON public.session_activities AS PERMISSIVE FOR SELECT TO public USING (true);
CREATE POLICY join_active_session_only ON public.session_students AS PERMISSIVE FOR INSERT TO public WITH CHECK ((EXISTS ( SELECT 1
   FROM sessions s
  WHERE ((s.id = session_students.session_id) AND (s.status = ANY (ARRAY['lobby'::text, 'active'::text, 'playing'::text]))))));
CREATE POLICY session_students_select_scoped ON public.session_students AS PERMISSIVE FOR SELECT TO public USING (((EXISTS ( SELECT 1
   FROM sessions s
  WHERE ((s.id = session_students.session_id) AND (s.teacher_id = ( SELECT auth.uid() AS uid))))) OR is_admin_user(( SELECT auth.uid() AS uid)) OR (EXISTS ( SELECT 1
   FROM sessions s
  WHERE ((s.id = session_students.session_id) AND (s.status <> 'ended'::text))))));
CREATE POLICY students_update_own_in_active_session ON public.session_students AS PERMISSIVE FOR UPDATE TO public USING ((EXISTS ( SELECT 1
   FROM sessions s
  WHERE ((s.id = session_students.session_id) AND (s.status <> 'ended'::text))))) WITH CHECK ((EXISTS ( SELECT 1
   FROM sessions s
  WHERE ((s.id = session_students.session_id) AND (s.status <> 'ended'::text)))));
CREATE POLICY sessions_delete_own ON public.sessions AS PERMISSIVE FOR DELETE TO public USING ((teacher_id = ( SELECT auth.uid() AS uid)));
CREATE POLICY sessions_insert_teacher ON public.sessions AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (((teacher_id = ( SELECT auth.uid() AS uid)) AND has_teacher_role() AND (get_effective_tier(( SELECT auth.uid() AS uid)) = ANY (ARRAY['trial'::text, 'pro'::text, 'admin'::text]))));
CREATE POLICY sessions_select ON public.sessions AS PERMISSIVE FOR SELECT TO public USING (((teacher_id = ( SELECT auth.uid() AS uid)) OR is_admin_user(( SELECT auth.uid() AS uid)) OR (status <> 'ended'::text)));
CREATE POLICY sessions_update_own ON public.sessions AS PERMISSIVE FOR UPDATE TO public USING ((teacher_id = ( SELECT auth.uid() AS uid)));
CREATE POLICY skill_state_select_admin ON public.skill_state AS PERMISSIVE FOR SELECT TO authenticated USING (is_admin_user(( SELECT auth.uid() AS uid)));
CREATE POLICY skill_state_history_select_admin ON public.skill_state_history AS PERMISSIVE FOR SELECT TO authenticated USING (is_admin_user(( SELECT auth.uid() AS uid)));
CREATE POLICY stripe_price_map_read ON public.stripe_price_map AS PERMISSIVE FOR SELECT TO anon, authenticated USING (active);
CREATE POLICY "Student reads own assignments" ON public.student_activity_assignments AS PERMISSIVE FOR SELECT TO anon USING ((student_id = auth.uid()));
CREATE POLICY "Teacher manages own student assignments" ON public.student_activity_assignments AS PERMISSIVE FOR ALL TO authenticated USING ((EXISTS ( SELECT 1
   FROM sessions s
...
CREATE POLICY insights_select_own ON public.teacher_insights AS PERMISSIVE FOR SELECT TO public USING (((teacher_id = ( SELECT auth.uid() AS uid)) OR COALESCE(( SELECT teachers.is_admin
   FROM teachers
  WHERE (teachers.id = ( SELECT auth.uid() AS uid))), false)));
CREATE POLICY insights_update_dismiss ON public.teacher_insights AS PERMISSIVE FOR UPDATE TO public USING ((teacher_id = ( SELECT auth.uid() AS uid)));
CREATE POLICY "teachers self-insert" ON public.teacher_profiles AS PERMISSIVE FOR INSERT TO public WITH CHECK ((auth.uid() = auth_user_id));
CREATE POLICY "teachers self-select" ON public.teacher_profiles AS PERMISSIVE FOR SELECT TO public USING ((auth.uid() = auth_user_id));
CREATE POLICY "teachers self-update" ON public.teacher_profiles AS PERMISSIVE FOR UPDATE TO public USING ((auth.uid() = auth_user_id)) WITH CHECK ((auth.uid() = auth_user_id));
CREATE POLICY "Teachers can update own row" ON public.teachers AS PERMISSIVE FOR UPDATE TO public USING ((( SELECT auth.uid() AS uid) = id));
CREATE POLICY teachers_insert_teacher_role ON public.teachers AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (((id = ( SELECT auth.uid() AS uid)) AND has_teacher_role()));
CREATE POLICY teachers_select_own_or_admin ON public.teachers AS PERMISSIVE FOR SELECT TO public USING (((id = ( SELECT auth.uid() AS uid)) OR is_admin_user(( SELECT auth.uid() AS uid))));
CREATE POLICY tenant_members_select_own ON public.tenant_members AS PERMISSIVE FOR SELECT TO authenticated USING ((user_id = ( SELECT auth.uid() AS uid)));
CREATE POLICY tenants_select_member ON public.tenants AS PERMISSIVE FOR SELECT TO authenticated USING ((EXISTS ( SELECT 1
...
-- ---------- grants: tables / views / sequences ----------
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.admin_alerts TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.admin_alerts TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.admin_alerts TO service_role;
...
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.analytics_events_real TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.analytics_events_real TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.analytics_events_real TO service_role;
...
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.broadcast_log TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.broadcast_log TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.broadcast_log TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.child_activity_summary TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.child_activity_summary TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.child_activity_summary TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.child_learning_state TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.child_learning_state TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.child_learning_state TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.child_profiles TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.child_profiles TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.child_profiles TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.class_children TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.class_children TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.class_children TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.class_session_tokens TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.class_session_tokens TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.class_session_tokens TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.classroom_default_activities TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.classroom_default_activities TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.classroom_default_activities TO service_role;
...
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.consent_records TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.consent_records TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.consent_records TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.dashboard_learner_progression TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.dashboard_learner_progression TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.dashboard_learner_progression TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.dashboard_trust_composition TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.dashboard_trust_composition TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.dashboard_trust_composition TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.data_deletion_requests TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.data_deletion_requests TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.data_deletion_requests TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.form_submissions TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.form_submissions TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.form_submissions TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.human_observation_fact TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.human_observation_fact TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.human_observation_fact TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.item_difficulty TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.item_difficulty TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.item_difficulty TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.join_audit_log TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.join_audit_log TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.join_rate_limits TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.learning_attempts TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.learning_attempts TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.learning_attempts TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.lios_adaptive_decisions TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.lios_adaptive_decisions TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.lios_adaptive_decisions TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.lios_anomaly_fact TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.lios_anomaly_fact TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.lios_anomaly_fact TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.lios_pipeline_runs TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.lios_pipeline_runs TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.lios_pipeline_runs TO service_role;
GRANT SELECT, UPDATE, USAGE ON SEQUENCE public.lios_pipeline_runs_id_seq TO anon;
GRANT SELECT, UPDATE, USAGE ON SEQUENCE public.lios_pipeline_runs_id_seq TO authenticated;
GRANT SELECT, UPDATE, USAGE ON SEQUENCE public.lios_pipeline_runs_id_seq TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.mastery_episode_fact TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.mastery_episode_fact TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.mastery_episode_fact TO service_role;
...
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.parent_controls TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.parent_controls TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.parent_controls TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.parent_profiles TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.parent_profiles TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.parent_profiles TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.parent_subscriptions TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.parent_subscriptions TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.parent_subscriptions TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.platform_admins TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.platform_insights TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.platform_insights TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.platform_insights TO service_role;
...
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.round_scores TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.round_scores TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.round_scores TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.school_invites TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.school_invites TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.school_invites TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.school_teachers TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.school_teachers TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.school_teachers TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.schools TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.schools TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.schools TO service_role;
GRANT MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE ON TABLE public.security_audit_log TO anon;
GRANT MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE ON TABLE public.security_audit_log TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.security_audit_log TO service_role;
GRANT SELECT, UPDATE, USAGE ON SEQUENCE public.security_audit_log_id_seq TO anon;
GRANT SELECT, UPDATE, USAGE ON SEQUENCE public.security_audit_log_id_seq TO authenticated;
GRANT SELECT, UPDATE, USAGE ON SEQUENCE public.security_audit_log_id_seq TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.session_activities TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.session_activities TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.session_activities TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.session_network_fingerprints TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.session_students TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.session_students TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.session_students TO service_role;
...
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.skill_state TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.skill_state TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.skill_state TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.skill_state_history TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.skill_state_history TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.skill_state_history TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.stripe_price_map TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.stripe_price_map TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.stripe_price_map TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.student_activity_assignments TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.student_activity_assignments TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.student_activity_assignments TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.teacher_insights TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.teacher_insights TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.teacher_insights TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.teacher_profiles TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.teacher_profiles TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.teacher_profiles TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.teachers TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.teachers TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.teachers TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.tenant_members TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.tenant_members TO authenticated;
...
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.v_activity_performance TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.v_activity_performance TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.v_activity_performance TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.v_engagement_metrics TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.v_engagement_metrics TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.v_engagement_metrics TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.v_growth_metrics TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.v_growth_metrics TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.v_growth_metrics TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.v_school_overview TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.v_school_overview TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.v_school_overview TO service_role;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.v_teacher_session_stats TO anon;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.v_teacher_session_stats TO authenticated;
GRANT DELETE, INSERT, MAINTAIN, REFERENCES, SELECT, TRIGGER, TRUNCATE, UPDATE ON TABLE public.v_teacher_session_stats TO service_role;

-- ---------- grants: columns ----------
GRANT SELECT (activity_version) ON public.sessions TO anon;

-- ---------- grants: functions (EXECUTE; revoke-then-grant to match proacl exactly) ----------
-- app_private.run_billing_health(): proacl NULL (default privileges)
-- app_private.run_email_dispatch(): proacl NULL (default privileges)
REVOKE ALL ON FUNCTION public._check_active_has_activity() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public._check_active_has_activity() TO PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public._class_assert_teacher(in_session_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public._class_assert_teacher(in_session_id uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public._ensure_tenant(in_kind text, in_owner uuid, in_name text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public._ensure_tenant(in_kind text, in_owner uuid, in_name text) TO service_role;
REVOKE ALL ON FUNCTION public._get_child_skills(p_child uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public._get_child_skills(p_child uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public._is_admin() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public._is_admin() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public._stamp_child_tenant() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public._stamp_child_tenant() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public._stamp_parent_profile_tenant() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public._stamp_parent_profile_tenant() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public._stamp_parent_tenant() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public._stamp_parent_tenant() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public._stamp_session_tenant() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public._stamp_session_tenant() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public._stamp_teacher_tenant() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public._stamp_teacher_tenant() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.analytics_events_promote_meta() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.analytics_events_promote_meta() TO PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.archive_child_profile(p_child uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.archive_child_profile(p_child uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.auth_owns_child(child uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.auth_owns_child(child uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.bump_child_activity_summary() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.bump_child_activity_summary() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.bump_child_learning_state() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.bump_child_learning_state() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.check_join_rate_limit(in_ip_hash text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.check_join_rate_limit(in_ip_hash text) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_assign_token(in_session_id uuid, in_class_child_id uuid, in_token text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_assign_token(in_session_id uuid, in_class_child_id uuid, in_token text) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_child_anonymise(in_class_child_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_child_anonymise(in_class_child_id uuid) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_child_archive(in_class_child_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_child_archive(in_class_child_id uuid) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_child_delete(in_class_child_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_child_delete(in_class_child_id uuid) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_delete_session(in_session_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_delete_session(in_session_id uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_end_activity(in_session_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_end_activity(in_session_id uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_end_session(in_session_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_end_session(in_session_id uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_end_stale_sessions() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_end_stale_sessions() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_get_activity(in_activity_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_get_activity(in_activity_id uuid) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_get_self(in_student_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_get_self(in_student_id uuid) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_get_session(in_session_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_get_session(in_session_id uuid) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_join(in_session_id uuid, in_name text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_join(in_session_id uuid, in_name text) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_join_by_ip(in_session_id uuid, in_name text, in_client_ip text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_join_by_ip(in_session_id uuid, in_name text, in_client_ip text) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_join_with_network(in_session_id uuid, in_name text, in_network_fingerprint text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_join_with_network(in_session_id uuid, in_name text, in_network_fingerprint text) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_join_with_token(in_session_id uuid, in_token text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_join_with_token(in_session_id uuid, in_token text) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_kick_student(in_session_id uuid, in_student_id uuid, in_reason text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_kick_student(in_session_id uuid, in_student_id uuid, in_reason text) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_pause_activity(in_session_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_pause_activity(in_session_id uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_resume_activity(in_session_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_resume_activity(in_session_id uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_set_readiness(in_student_id uuid, in_state text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_set_readiness(in_student_id uuid, in_state text) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_set_scoreboard_visibility(in_session_id uuid, in_visible boolean) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_set_scoreboard_visibility(in_session_id uuid, in_visible boolean) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_start_activity(in_session_id uuid, in_activity text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_start_activity(in_session_id uuid, in_activity text) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_student_heartbeat(in_student_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_student_heartbeat(in_student_id uuid) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_student_stats(in_session_id uuid, in_student_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_student_stats(in_session_id uuid, in_student_id uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_summary(in_session_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_summary(in_session_id uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.class_validate_join(in_code text, in_network_fingerprint text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_validate_join(in_code text, in_network_fingerprint text) TO authenticated, service_role, anon;
REVOKE ALL ON FUNCTION public.class_validate_join_by_ip(in_code text, in_client_ip text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.class_validate_join_by_ip(in_code text, in_client_ip text) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.compute_billing_preview(p_interval text, p_active_children integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.compute_billing_preview(p_interval text, p_active_children integer) TO PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.create_child_profile(p_nickname text, p_age_band text, p_learning_focus text, p_avatar text, p_preferred_hand text, p_consent_version text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.create_child_profile(p_nickname text, p_age_band text, p_learning_focus text, p_avatar text, p_preferred_hand text, p_consent_version text) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_ab_results(in_flag text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_ab_results(in_flag text) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_adaptive_decisions(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_adaptive_decisions(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_anomaly_check() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_anomaly_check() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_classrooms(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_classrooms(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_cohort_curves(in_weeks integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_cohort_curves(in_weeks integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_cohort_retention(in_weeks integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_cohort_retention(in_weeks integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_context_split(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_context_split(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_curriculum_coverage(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_curriculum_coverage(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_daily_digest() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_daily_digest() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_engagement_deep(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_engagement_deep(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_errors(row_limit integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_errors(row_limit integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_executive_summary(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_executive_summary(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_export_headline(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_export_headline(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_friction_engineering(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_friction_engineering(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_funnel(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_funnel(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_gesture_quality(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_gesture_quality(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_growth(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_growth(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_ingest_latency(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_ingest_latency(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_latest_sessions(row_limit integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_latest_sessions(row_limit integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_live() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_live() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_mastery(in_days integer, in_min_attempts integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_mastery(in_days integer, in_min_attempts integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_mastery_milestones(in_days integer, in_min_attempts integer, in_threshold_pct integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_mastery_milestones(in_days integer, in_min_attempts integer, in_threshold_pct integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_mastery_summary(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_mastery_summary(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_mastery_v2(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_mastery_v2(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_observability(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_observability(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_observations(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_observations(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_pipeline_status(in_limit integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_pipeline_status(in_limit integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_progression_for_learner(in_device_id text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_progression_for_learner(in_device_id text) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_progression_top_learners(in_days integer, in_limit integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_progression_top_learners(in_days integer, in_limit integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_public_proof() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_public_proof() TO PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_retention_deep() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_retention_deep() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_today() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_today() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_top_modes(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_top_modes(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_tracker_health(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_tracker_health(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_transparency_report(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_transparency_report(in_days integer) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_transparency_signals(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_transparency_signals(in_days integer) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.dashboard_trust_strip(in_days integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.dashboard_trust_strip(in_days integer) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.export_family_data() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.export_family_data() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.export_parent_data() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.export_parent_data() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.generate_network_fingerprint(in_ip text, in_secret text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.generate_network_fingerprint(in_ip text, in_secret text) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.get_account_roles() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.get_account_roles() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.get_child_dashboard(p_child uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.get_child_dashboard(p_child uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.get_effective_tier(teacher_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.get_effective_tier(teacher_id uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.get_email_cron_key() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.get_email_cron_key() TO service_role;
REVOKE ALL ON FUNCTION public.get_parent_overview() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.get_parent_overview() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.get_student_assignments(in_student_id uuid, in_session_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.get_student_assignments(in_student_id uuid, in_session_id uuid) TO authenticated, service_role, anon;
REVOKE ALL ON FUNCTION public.get_student_class_state(in_student_id uuid, in_session_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.get_student_class_state(in_student_id uuid, in_session_id uuid) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.get_weekly_summary(p_child uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.get_weekly_summary(p_child uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.handle_new_parent_user() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.handle_new_parent_user() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.handle_new_teacher_user() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.handle_new_teacher_user() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.handle_new_user() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.handle_new_user() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.has_parent_role() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.has_parent_role() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.has_teacher_role() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.has_teacher_role() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.ingest_analytics_events(in_events jsonb) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.ingest_analytics_events(in_events jsonb) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.ingest_learning_attempts(in_attempts jsonb) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.ingest_learning_attempts(in_attempts jsonb) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.initialize_teacher_trial() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.initialize_teacher_trial() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.is_admin_user(check_user_id uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.is_admin_user(check_user_id uuid) TO authenticated, service_role, anon;
REVOKE ALL ON FUNCTION public.is_platform_admin() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.is_platform_admin() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.is_school_admin(in_tenant uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.is_school_admin(in_tenant uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.landing_public_proof() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.landing_public_proof() TO PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.learning_attempts_promote_meta() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.learning_attempts_promote_meta() TO PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.lios_detect_anomalies_v1() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.lios_detect_anomalies_v1() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.lios_detect_friction_v1(p_lookback interval) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.lios_detect_friction_v1(p_lookback interval) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.lios_detect_mastery_episodes_v1() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.lios_detect_mastery_episodes_v1() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.lios_detect_mastery_episodes_v1(p_lookback interval) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.lios_detect_mastery_episodes_v1(p_lookback interval) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.lios_is_technical_item_key(k text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.lios_is_technical_item_key(k text) TO PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.lios_recommend_next(p_device_id text, p_session_id uuid, p_game_mode text, p_current_item text, p_was_correct boolean) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.lios_recommend_next(p_device_id text, p_session_id uuid, p_game_mode text, p_current_item text, p_was_correct boolean) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.lios_record_observation(p_device_id text, p_focus_tags text[], p_affect_tags text[], p_independence_tags text[], p_social_tags text[], p_notable_tags text[], p_session_id uuid, p_classroom_code text, p_age_band text, p_note text, p_observer_role text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.lios_record_observation(p_device_id text, p_focus_tags text[], p_affect_tags text[], p_independence_tags text[], p_social_tags text[], p_notable_tags text[], p_session_id uuid, p_classroom_code text, p_age_band text, p_note text, p_observer_role text) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.lios_run_pipeline(p_lookback interval) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.lios_run_pipeline(p_lookback interval) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.lios_score_unscored_attempts(p_lookback interval) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.lios_score_unscored_attempts(p_lookback interval) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.lios_update_elo_v1(p_lookback interval) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.lios_update_elo_v1(p_lookback interval) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.log_security_event(in_event_type text, in_metadata jsonb, in_ip_hash text, in_ua_hash text, in_country text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.log_security_event(in_event_type text, in_metadata jsonb, in_ip_hash text, in_ua_hash text, in_country text) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.normalize_ip_for_network(in_ip text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.normalize_ip_for_network(in_ip text) TO anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.parent_has_access(p_parent uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.parent_has_access(p_parent uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.parent_subscription_state(p_parent uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.parent_subscription_state(p_parent uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.pricing_amount_cents(p_interval text, p_active_children integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.pricing_amount_cents(p_interval text, p_active_children integer) TO PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.process_account_deletion_requests(p_limit integer) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.process_account_deletion_requests(p_limit integer) TO service_role;
REVOKE ALL ON FUNCTION public.record_consent(p_consent_type text, p_consent_version text, p_granted boolean) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.record_consent(p_consent_type text, p_consent_version text, p_granted boolean) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.register_parent() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.register_parent() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.register_teacher(in_full_name text, in_school_name text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.register_teacher(in_full_name text, in_school_name text) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.request_account_deletion() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.request_account_deletion() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.request_child_deletion(p_child uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.request_child_deletion(p_child uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.restore_child_profile(p_child uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.restore_child_profile(p_child uuid) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.session_lookup_by_code(in_code text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.session_lookup_by_code(in_code text) TO authenticated, service_role, anon;
REVOKE ALL ON FUNCTION public.session_set_network_fingerprint(in_session_id uuid, in_teacher_ip text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.session_set_network_fingerprint(in_session_id uuid, in_teacher_ip text) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.session_validate_network(in_session_id uuid, in_network_fingerprint text) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.session_validate_network(in_session_id uuid, in_network_fingerprint text) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.set_classroom_default_activities(in_session_id uuid, in_activities text[]) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.set_classroom_default_activities(in_session_id uuid, in_activities text[]) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.set_session_school_id() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.set_session_school_id() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.set_student_assignments(in_session_id uuid, in_student_id uuid, in_activities jsonb) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.set_student_assignments(in_session_id uuid, in_student_id uuid, in_activities jsonb) TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.set_updated_at() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.set_updated_at() TO PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.start_parent_trial(p_parent uuid) FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.start_parent_trial(p_parent uuid) TO service_role;
REVOKE ALL ON FUNCTION public.sync_teacher_school_tier() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.sync_teacher_school_tier() TO authenticated, service_role;
REVOKE ALL ON FUNCTION public.touch_class_children_updated_at() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.touch_class_children_updated_at() TO PUBLIC, anon, authenticated, service_role;
REVOKE ALL ON FUNCTION public.user_tenant_ids() FROM PUBLIC, anon, authenticated, service_role; GRANT EXECUTE ON FUNCTION public.user_tenant_ids() TO authenticated, service_role;
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname='supabase_realtime' AND schemaname='public' AND tablename='sessions') THEN ALTER PUBLICATION supabase_realtime ADD TABLE public.sessions; END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname='supabase_realtime' AND schemaname='public' AND tablename='session_students') THEN ALTER PUBLICATION supabase_realtime ADD TABLE public.session_students; END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname='supabase_realtime' AND schemaname='public' AND tablename='round_scores') THEN ALTER PUBLICATION supabase_realtime ADD TABLE public.round_scores; END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_publication_tables WHERE pubname='supabase_realtime' AND schemaname='public' AND tablename='session_activities') THEN ALTER PUBLICATION supabase_realtime ADD TABLE public.session_activities; END IF;
END $pub$;

-- ---------- cron jobs (9; guarded so re-running never duplicates) ----------
DO $cron$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'refresh-materialized-views') THEN
    PERFORM cron.schedule('refresh-materialized-views', '0 3 * * *', '
  ...
  ');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'retention-purge') THEN
    PERFORM cron.schedule('retention-purge', '0 2 * * *', '
  ...
  ');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'dita-prune-analytics-events') THEN
    PERFORM cron.schedule('dita-prune-analytics-events', '0 3 * * *', 'DELETE FROM public.analytics_events WHERE occurred_at < now() - interval ''365 days''');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'dita-prune-learning-attempts') THEN
    PERFORM cron.schedule('dita-prune-learning-attempts', '5 3 * * *', 'DELETE FROM public.learning_attempts WHERE occurred_at < now() - interval ''365 days''');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'dita-daily-digest') THEN
    PERFORM cron.schedule('dita-daily-digest', '0 7 * * *', '
    ...
    ');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'dita-anomaly-check') THEN
    PERFORM cron.schedule('dita-anomaly-check', '*/15 * * * *', '
    SELECT net.http_post(
    ...
    ');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'lios-pipeline-every-5min') THEN
    PERFORM cron.schedule('lios-pipeline-every-5min', '*/5 * * * *', 'SELECT public.lios_run_pipeline(''15 minutes''::interval);');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'email-dispatch-15m') THEN
    PERFORM cron.schedule('email-dispatch-15m', '*/15 * * * *', 'select app_private.run_email_dispatch()');
  END IF;
  IF NOT EXISTS (SELECT 1 FROM cron.job WHERE jobname = 'billing-health-15m') THEN
    PERFORM cron.schedule('billing-health-15m', '7-59/15 * * * *', 'select app_private.run_billing_health()');
  END IF;
END $cron$;

-- ---------- reference: default ACLs and extension versions observed in production (informational) ----------
-- defacl role=postgres schema=public type=r acl={postgres=arwdDxtm/postgres,anon=arwdDxtm/postgres,authenticated=arwdDxtm/postgres,service_role=arwdDxtm/postgres}
-- defacl role=postgres schema=public type=f acl={postgres=X/postgres,anon=X/postgres,authenticated=X/postgres,service_role=X/postgres}
-- defacl role=postgres schema=public type=S acl={postgres=rwU/postgres,anon=rwU/postgres,authenticated=rwU/postgres,service_role=rwU/postgres}
-- defacl role=supabase_admin schema=public type=S acl={postgres=rwU/supabase_admin,anon=rwU/supabase_admin,authenticated=rwU/supabase_admin,service_role=rwU/supabase_admin}
-- defacl role=supabase_admin schema=public type=r acl={postgres=arwdDxtm/supabase_admin,anon=arwdDxtm/supabase_admin,authenticated=arwdDxtm/supabase_admin,service_role=arwdDxtm/supabase_admin}
-- defacl role=supabase_admin schema=public type=f acl={postgres=X/supabase_admin,anon=X/supabase_admin,authenticated=X/supabase_admin,service_role=X/supabase_admin}
-- defacl role=postgres schema=storage type=r acl={postgres=arwdDxtm/postgres,anon=arwdDxtm/postgres,authenticated=arwdDxtm/postgres,service_role=arwdDxtm/postgres}
-- defacl role=postgres schema=storage type=f acl={postgres=X/postgres,anon=X/postgres,authenticated=X/postgres,service_role=X/postgres}
-- defacl role=postgres schema=storage type=S acl={postgres=rwU/postgres,anon=rwU/postgres,authenticated=rwU/postgres,service_role=rwU/postgres}
-- defacl role=supabase_auth_admin schema=auth type=r acl={postgres=arwdDxtm/supabase_auth_admin,dashboard_user=arwdDxtm/supabase_auth_admin}
-- defacl role=supabase_auth_admin schema=auth type=S acl={postgres=rwU/supabase_auth_admin,dashboard_user=rwU/supabase_auth_admin}
-- defacl role=supabase_auth_admin schema=auth type=f acl={postgres=X/supabase_auth_admin,dashboard_user=X/supabase_auth_admin}
-- defacl role=supabase_admin schema=realtime type=r acl={postgres=arwdDxtm/supabase_admin,dashboard_user=arwdDxtm/supabase_admin}
-- defacl role=supabase_admin schema=realtime type=S acl={postgres=rwU/supabase_admin,dashboard_user=rwU/supabase_admin}
-- defacl role=supabase_admin schema=realtime type=f acl={postgres=X/supabase_admin,dashboard_user=X/supabase_admin}
-- defacl role=supabase_admin schema=graphql_public type=S acl={postgres=rwU/supabase_admin,anon=rwU/supabase_admin,authenticated=rwU/supabase_admin,service_role=rwU/supabase_admin}
-- defacl role=supabase_admin schema=graphql_public type=r acl={postgres=arwdDxtm/supabase_admin,anon=arwdDxtm/supabase_admin,authenticated=arwdDxtm/supabase_admin,service_role=arwdDxtm/supabase_admin}
-- defacl role=supabase_admin schema=graphql_public type=f acl={postgres=X/supabase_admin,anon=X/supabase_admin,authenticated=X/supabase_admin,service_role=X/supabase_admin}
-- defacl role=supabase_admin schema=extensions type=r acl={postgres=a*r*w*d*D*x*t*m*/supabase_admin}
-- defacl role=supabase_admin schema=extensions type=f acl={postgres=X*/supabase_admin}
-- defacl role=supabase_admin schema=extensions type=S acl={postgres=r*w*U*/supabase_admin}
-- defacl role=supabase_admin schema=graphql type=r acl={postgres=arwdDxtm/supabase_admin,anon=arwdDxtm/supabase_admin,authenticated=arwdDxtm/supabase_admin,service_role=arwdDxtm/supabase_admin}
-- defacl role=supabase_admin schema=graphql type=f acl={postgres=X/supabase_admin,anon=X/supabase_admin,authenticated=X/supabase_admin,service_role=X/supabase_admin}
-- defacl role=supabase_admin schema=graphql type=S acl={postgres=rwU/supabase_admin,anon=rwU/supabase_admin,authenticated=rwU/supabase_admin,service_role=rwU/supabase_admin}
-- defacl role=supabase_admin schema=cron type=S acl={postgres=r*w*U*/supabase_admin}
-- defacl role=supabase_admin schema=cron type=r acl={postgres=a*r*w*d*D*x*t*m*/supabase_admin}
-- defacl role=supabase_admin schema=cron type=f acl={postgres=X*/supabase_admin}
-- ext plpgsql 1.0 schema=pg_catalog
-- ext pg_stat_statements 1.11 schema=extensions
-- ext uuid-ossp 1.1 schema=extensions
-- ext pgcrypto 1.3 schema=extensions
-- ext supabase_vault 0.3.1 schema=vault
-- ext pg_cron 1.6.4 schema=pg_catalog
-- ext pg_net 0.19.5 schema=extensions
