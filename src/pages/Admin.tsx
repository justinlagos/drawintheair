...
export const Admin: React.FC = () => {
  ...
  // Check if already authenticated (session in memory)
  useEffect(() => {
  ...
  const loadData = async () => {
    try {
      ...
      setKpiData({
        total_visits: analyticsEvents.filter((e: any) => e.event_name === 'landing_view').length,
        unique_sessions: sessions.size,
        try_demo_clicks: analyticsEvents.filter((e: any) => e.event_name === 'demo_try_click').length,
      ...
      <div className="admin-content">
        ...
        <div className="admin-kpi-strip">
          <div className="admin-kpi-card">
            <div className="admin-kpi-value">{kpiData.total_visits}</div>
            <div className="admin-kpi-label">Total visits</div>
          </div>
          <div className="admin-kpi-card">
            <div className="admin-kpi-value">{kpiData.unique_sessions}</div>
            <div className="admin-kpi-label">Unique sessions</div>
          </div>
          <div className="admin-kpi-card">
            <div className="admin-kpi-value">{kpiData.try_demo_clicks}</div>
            <div className="admin-kpi-label">Try demo clicks</div>
          </div>
          <div className="admin-kpi-card">
            <div className="admin-kpi-value">{kpiData.demo_sessions_started}</div>
            <div className="admin-kpi-label">Demo sessions started</div>
          </div>
        </div>
            ...
            {funnelSteps.map((step, index) => (
              <div key={index} className="admin-funnel-step">
                ...
                <div className="admin-funnel-step-meta">
                  <span>{step.percent_of_previous.toFixed(1)}% of previous</span>
                  <span>{step.percent_of_total.toFixed(1)}% of total</span>
                </div>
              </div>
            ))}
          </div>
