...
export default function ForParentsPage() {
  ...
  return (
    <SeoLayout>
      ...
      {/* Why parents love it */}
      <Section light>
        <h2 className="h3" style={{ marginBottom: 8, marginTop: 0 }}>Why Parents Choose Draw in the Air</h2>
        <p style={{ color: 'var(--ink-500, #6B6580)', marginBottom: 28, fontSize: '0.92rem' }}>Thousands of families use Draw in the Air every week. Here's what makes it different from other kids' apps.</p>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 16 }}>
          {BENEFITS.map(b => (
            <div key={b.title} className="card" style={{ padding: 20 }}>
              <div style={{ fontSize: '1.8rem', marginBottom: 8 }} aria-hidden="true">{b.icon}</div>
              <div style={{ color: 'var(--ink-900, #1F1B2E)', fontWeight: 700, fontSize: '0.95rem', marginBottom: 6, fontFamily: 'var(--font-display)' }}>{b.title}</div>
              <div style={{ color: 'var(--ink-500, #6B6580)', fontSize: '0.82rem', lineHeight: 1.6 }}>{b.desc}</div>
            </div>
          ))}
        </div>
      </Section>
      ...
      {/* What children learn */}
      <Section>
        <h2 className="h3" style={{ marginBottom: 8, marginTop: 0 }}>What Your Child Will Learn</h2>
        <p style={{ color: 'var(--ink-500, #6B6580)', marginBottom: 28, fontSize: '0.92rem' }}>Draw in the Air isn't just entertainment, every activity is designed to build measurable developmental skills.</p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
          {[
            ...
            <div key={item.activity} style={{ borderBottom: '1px solid var(--border-2, rgba(31,27,46,0.10))', padding: '20px 0', display: 'grid', gridTemplateColumns: 'auto 1fr', gap: 20, alignItems: 'start' }}>
              <div style={{ fontSize: '2rem' }} aria-hidden="true">{item.icon}</div>
              <div>
                <div style={{ color: 'var(--ink-900, #1F1B2E)', fontWeight: 700, marginBottom: 8, fontFamily: 'var(--font-display)' }}>{item.activity}</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {item.skills.map(s => <span key={s} style={{ background: 'var(--lavender-50, #F4EFFF)', color: 'var(--lavender-700, #5C3FB0)', padding: '3px 10px', borderRadius: 20, fontSize: '0.75rem', fontWeight: 600 }}>{s}</span>)}
                </div>
              </div>
            </div>
          ))}
      ...
      {/* How to get started */}
      <Section light>
        <h2 className="h3" style={{ marginBottom: 20, marginTop: 0 }}>Getting Started in 60 Seconds</h2>
        {[
          ...
          <div key={i} style={{ display: 'flex', gap: 16, marginBottom: 20, alignItems: 'flex-start' }}>
            <div style={{ width: 32, height: 32, borderRadius: '50%', background: 'linear-gradient(180deg, #9D7DFF 0%, #8A66F0 100%)', color: '#FFFFFF', fontWeight: 800, fontFamily: 'var(--font-display)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{i + 1}</div>
            <div>
              <div style={{ color: 'var(--ink-900, #1F1B2E)', fontWeight: 700, marginBottom: 4, fontSize: '0.95rem', fontFamily: 'var(--font-display)' }}>{step}</div>
              <div style={{ color: 'var(--ink-500, #6B6580)', fontSize: '0.87rem', lineHeight: 1.6 }}>{desc}</div>
            </div>
          </div>
        ))}
        <div style={{ marginTop: 28, textAlign: 'center' }}>
          <button className="btn btn-primary lg" onClick={() => navigate(SITE.appPath)}>
            Try It Now, Free ✨
          </button>
        </div>
      </Section>
      ...
      {/* Also useful for */}
      <Section>
        <h2 className="h3" style={{ marginBottom: 20, marginTop: 0 }}>Also Useful For</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12 }}>
          {[
            ...
            <button key={item.path} className="card hoverable" onClick={() => navigate(item.path)} style={{ padding: 16, color: 'var(--ink-900, #1F1B2E)', display: 'flex', alignItems: 'center', gap: 10, fontWeight: 600, fontSize: '0.88rem', fontFamily: 'var(--font-display)', textAlign: 'left' }}>
              <span style={{ fontSize: '1.4rem' }} aria-hidden="true">{item.icon}</span>{item.label}
            </button>
          ))}
        </div>
      </Section>
      ...
      <Section light>
        <h2 className="h3" style={{ marginBottom: 24, marginTop: 0 }}>Parent FAQ</h2>
