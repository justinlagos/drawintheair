...
function EducationalContent({ type, value }: { type: 'letter' | 'number' | 'shape'; value: string }) {
  ...
  if (type === 'letter') {
    ...
    return (
      <div>
        <h2 style={{ color: '#1F1B2E', fontSize: '1.35rem', fontWeight: 800, marginBottom: 20 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
        {currentType !== 'letter' && (
          <button style={btnStyle} onClick={() => navigate('/trace-a')}
            onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = '#8A66F0'; }}
...
function RelatedCard({ label, path, emoji }: { label: string; path: string; emoji: string }) {
  return (
    <button onClick={() => navigate(path)} style={{
      ...
      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = 'rgba(138,102,240,0.18)'; (e.currentTarget as HTMLElement).style.background = 'rgba(138,102,240,0.08)'; }}
    >
    ...
    <SeoLayout>
      ...
      <Section>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 32 }}>
          <div>
            <h2 style={{ color: '#1F1B2E', fontSize: '1.2rem', fontWeight: 800, marginBottom: 16 }}>Quick Facts</h2>
            {type === 'letter' && (() => {
              const L = value.toUpperCase();
              const ph = PHONICS[L] || { sound: '', word: L, emoji: '✏️' };
              return (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {[['Letter', `${L} / ${L.toLowerCase()}`], ['Phonics Sound', ph.sound], ['Example Word', `${ph.word} ${ph.emoji}`], ['Position', `#${LETTERS.indexOf(L) + 1} of 26`]].map(([k, v]) => (
                    <div key={String(k)} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                      <span style={{ color: '#8A66F0', fontSize: '0.78rem', fontWeight: 700, minWidth: 110 }}>{k}</span>
                      <span style={{ color: '#1F1B2E', fontSize: '0.88rem' }}>{v}</span>
                    </div>
                  ))}
                </div>
              );
              ...
              return (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {[['Number', `${value} (${cap(m.word)})`], ['Emoji', m.emoji], ['Fun Fact', m.funFact]].map(([k, v]) => (
                    <div key={String(k)} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                      <span style={{ color: '#8A66F0', fontSize: '0.78rem', fontWeight: 700, minWidth: 80 }}>{k}</span>
                      <span style={{ color: '#1F1B2E', fontSize: '0.88rem' }}>{v}</span>
                    </div>
                  ))}
                </div>
              ...
              return (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {[['Shape', cap(value)], ['Properties', m.sides], ['Description', m.description]].map(([k, v]) => (
                    <div key={String(k)} style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                      <span style={{ color: '#8A66F0', fontSize: '0.78rem', fontWeight: 700, minWidth: 100 }}>{k}</span>
                      <span style={{ color: '#1F1B2E', fontSize: '0.88rem' }}>{v}</span>
                    </div>
                  ))}
                </div>
          ...
          <div>
            <h2 style={{ color: '#1F1B2E', fontSize: '1.2rem', fontWeight: 800, marginBottom: 16 }}>How to Trace</h2>
            {[
              ...
              <div key={String(step)} style={{ display: 'flex', gap: 12, marginBottom: 12 }}>
                <span style={{ color: '#8A66F0', fontWeight: 800, fontSize: '0.8rem', minWidth: 110 }}>{step}</span>
