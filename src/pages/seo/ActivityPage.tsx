...
export default function ActivityPage({ slug }: ActivityPageProps) {
  ...
  return (
    <SeoLayout>
      ...
      <Section light>
        <h2 style={{ color: '#1F1B2E', fontSize: '1.4rem', fontWeight: 800, marginBottom: 24 }}>Skills This Activity Develops</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16 }}>
          {skills.map(s => (
            <div key={s.title} style={{ background: 'rgba(138,102,240,0.08)', border: '1px solid rgba(138,102,240,0.17)', borderRadius: 12, padding: 20 }}>
              <div style={{ fontSize: '2rem', marginBottom: 8 }}>{s.icon}</div>
              <div style={{ color: '#1F1B2E', fontWeight: 700, marginBottom: 4 }}>{s.title}</div>
      ...
      <Section>
        <h2 style={{ color: '#1F1B2E', fontSize: '1.4rem', fontWeight: 800, marginBottom: 20 }}>How to Play</h2>
        {howItWorks.map((step, i) => (
          <div key={i} style={{ display: 'flex', gap: 16, marginBottom: 16, alignItems: 'flex-start' }}>
            <div style={{ width: 28, height: 28, borderRadius: '50%', background: '#8A66F0', color: '#1F1B2E', fontWeight: 800, fontSize: '0.82rem', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{i + 1}</div>
            <p style={{ color: '#1F1B2E', fontSize: '0.9rem', lineHeight: 1.6, margin: 0 }}>{step}</p>
          </div>
        ))}
      </Section>
      ...
      <Section>
        <h2 style={{ color: '#1F1B2E', fontSize: '1.4rem', fontWeight: 800, marginBottom: 20 }}>Try Other Activities</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 12 }}>
          {[
            ...
            <button key={a.path} onClick={() => navigate(a.path)} style={{ background: '#FFFFFF', border: '1px solid rgba(63,64,82,0.14)', borderRadius: 10, padding: '14px 12px', cursor: 'pointer', color: '#1F1B2E', fontWeight: 600, fontSize: '0.85rem' }}>
              {a.label}
            </button>
          ))}
        </div>
