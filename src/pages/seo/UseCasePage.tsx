...
// ─── Styles ───────────────────────────────────────────────────────────────────
const styles = {
  h2: { color: '#1F1B2E', fontSize: '1.6rem', fontWeight: 800, marginBottom: 20 } as React.CSSProperties,
  h3: { color: '#1F1B2E', fontSize: '1.2rem', fontWeight: 700, marginBottom: 12 } as React.CSSProperties,
  ...
  card: { background: '#FFFFFF', border: '1px solid rgba(138,102,240,0.17)', borderRadius: 12, padding: '20px 24px' } as React.CSSProperties,
  ...
  return (
    <SeoLayout>
        ...
        <div style={styles.grid2}>
          {data.classroomExamples.map((ex, i) => (
            <div key={i} style={styles.card}>
              ...
              <div style={{ color: '#1F1B2E', fontWeight: 700, marginBottom: 6, fontSize: '1rem' }}>{ex.title}</div>
      ...
      <Section light>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {data.steps.map((step, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
              ...
              <p style={{ color: '#1F1B2E', fontSize: '0.95rem', lineHeight: 1.6, margin: 0, paddingTop: 4 }}>{step.text}</p>
            </div>
          ))}
        </div>
        <div style={{ marginTop: 32, textAlign: 'center' }}>
          <button
            ...
            style={{ background: 'linear-gradient(180deg, #9D7DFF 0%, #8A66F0 100%)', color: '#1F1B2E', border: 'none', borderRadius: 32, padding: '14px 36px', fontWeight: 800, fontSize: '1rem', cursor: 'pointer' }}
          >
      ...
      <Section light>
        ...
        <h3 style={{ ...styles.h3, color: '#7A55E0', marginBottom: 12 }}>Continue learning</h3>
        <div style={{ ...styles.linkRow, marginBottom: 28 }}>
          {data.internalLinks.map(link => (
            <button
              key={link.path}
              onClick={() => navigate(link.path)}
              style={{ background: 'rgba(138,102,240,0.10)', border: '1px solid rgba(138,102,240,0.297)', color: '#7A55E0', borderRadius: 20, padding: '8px 18px', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer' }}
            >
              {link.label}
            </button>
          ))}
        </div>
        <h3 style={{ ...styles.h3, color: '#2E9D68', marginBottom: 12 }}>Also useful</h3>
        <div style={styles.linkRow}>
          {data.relatedLinks.map(link => (
            <button
              key={link.path}
              onClick={() => navigate(link.path)}
              style={{ background: 'rgba(90,153,242,0.16)', border: '1px solid rgba(46,157,104,0.3)', color: '#2E9D68', borderRadius: 20, padding: '8px 18px', fontSize: '0.85rem', fontWeight: 600, cursor: 'pointer' }}
            >
              {link.label}
            </button>
          ))}
