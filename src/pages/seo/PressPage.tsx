    ...
    <SeoLayout>
      ...
      <Section>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <h2 style={{ color: '#1A1B2E', fontSize: '1.6rem', fontWeight: 800, marginBottom: 20 }}>
            About Draw in the Air
          </h2>
          <p style={{ color: '#4A4D6B', fontSize: '1.05rem', lineHeight: 1.7, marginBottom: 16 }}>
          ...
          <p style={{ color: '#4A4D6B', fontSize: '1.05rem', lineHeight: 1.7, marginBottom: 32 }}>
          ...
          {/* Key facts grid */}
          <h3 style={{ color: '#1A1B2E', fontSize: '1.2rem', fontWeight: 700, marginBottom: 16 }}>Key Facts</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginBottom: 40 }}>
            {KEY_FACTS.map(f => (
              <div key={f.label} style={{ background: 'rgba(3,78,174,0.06)', border: '1px solid rgba(3,78,174,0.17)', borderRadius: 10, padding: '14px 16px' }}>
                <div style={{ color: '#6C3FA4', fontSize: '0.75rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>{f.label}</div>
                <div style={{ color: '#1A1B2E', fontSize: '1rem', fontWeight: 700 }}>{f.value}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <Section light>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <h2 style={{ color: '#1A1B2E', fontSize: '1.6rem', fontWeight: 800, marginBottom: 20 }}>Brand Assets</h2>
          <p style={{ color: '#4A4D6B', marginBottom: 24, fontSize: '0.95rem' }}>Download logos and product screenshots for editorial use. All assets may be used in coverage of Draw in the Air with appropriate attribution.</p>

          <h3 style={{ color: '#1A1B2E', fontSize: '1.1rem', fontWeight: 700, marginBottom: 16 }}>Logos</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 20, marginBottom: 32 }}>
            <div style={{ background: '#FFFAEB', padding: 30, borderRadius: 14, border: '1px solid rgba(3,78,174,0.17)', textAlign: 'center' }}>
              <img src="/logo.svg" alt="Draw in the Air Primary Logo" style={{ height: 80, width: 'auto', marginBottom: 12 }} />
              <div style={{ color: '#1A1B2E', fontWeight: 700, fontSize: '0.9rem' }}>Primary Logo (Cream BG)</div>
              <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginTop: 12 }}>
                <a href="/logo.svg" download style={{ color: '#034eae', textDecoration: 'none', fontSize: '0.82rem', fontWeight: 600 }}>Download SVG</a>
                <span style={{ color: '#6B6F84', fontSize: '0.82rem' }}>·</span>
                <a href="/logo.png" download style={{ color: '#1c7e80', textDecoration: 'none', fontSize: '0.82rem', fontWeight: 600 }}>Download PNG</a>
              </div>
            </div>
            <div style={{ background: '#1A1B2E', padding: 30, borderRadius: 14, border: '1px solid rgba(3,78,174,0.17)', textAlign: 'center' }}>
              <img src="/logo.svg" alt="Draw in the Air Logo on Dark Background" style={{ height: 80, width: 'auto', marginBottom: 12 }} />
              <div style={{ color: '#FFFAEB', fontWeight: 700, fontSize: '0.9rem' }}>Logo on Dark BG</div>
              <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginTop: 12 }}>
                <a href="/logo.svg" download style={{ color: '#FFD84D', textDecoration: 'none', fontSize: '0.82rem', fontWeight: 600 }}>Download SVG</a>
                <span style={{ color: '#94a3b8', fontSize: '0.82rem' }}>·</span>
                <a href="/logo.png" download style={{ color: '#FFD84D', textDecoration: 'none', fontSize: '0.82rem', fontWeight: 600 }}>Download PNG</a>
              </div>
            </div>
          </div>

          <h3 style={{ color: '#1A1B2E', fontSize: '1.1rem', fontWeight: 700, marginBottom: 16 }}>Product Screenshots</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16, marginBottom: 32 }}>
            {PRODUCT_SCREENSHOTS.map(s => (
              <div key={s.label} style={{ background: '#FFFFFF', borderRadius: 12, overflow: 'hidden', border: '1px solid rgba(3,78,174,0.128)' }}>
                <img src={s.src} alt={s.alt} style={{ width: '100%', height: 'auto', display: 'block' }} loading="lazy" />
                <div style={{ padding: '10px 14px' }}>
                  <div style={{ color: '#1A1B2E', fontWeight: 600, fontSize: '0.85rem' }}>{s.label}</div>
                </div>
              </div>
            ))}
          </div>

          <h3 style={{ color: '#1A1B2E', fontSize: '1.1rem', fontWeight: 700, marginBottom: 12 }}>Brand Colours</h3>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 32 }}>
            {[
              { name: 'Brand Blue', hex: '#034eae' },
              { name: 'Sunshine', hex: '#f9e55e' },
              { name: 'Plum',       hex: '#6C3FA4' },
              { name: 'Aqua',       hex: '#55DDE0' },
              { name: 'Coral',      hex: '#FF6B6B' },
              { name: 'Cream',      hex: '#FFFAEB' },
              { name: 'Ink',        hex: '#1A1B2E' },
            ].map(c => (
              <div key={c.name} style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#FFFFFF', border: '1px solid rgba(63,64,82,0.14)', borderRadius: 8, padding: '8px 14px' }}>
                <div style={{ width: 24, height: 24, borderRadius: 6, background: c.hex, flexShrink: 0 }} />
                <div>
                  <div style={{ color: '#1A1B2E', fontWeight: 600, fontSize: '0.82rem' }}>{c.name}</div>
                  <div style={{ color: '#4A4D6B', fontSize: '0.72rem', fontFamily: 'monospace' }}>{c.hex}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <Section>
        <div style={{ maxWidth: 800, margin: '0 auto' }}>
          <h2 style={{ color: '#1A1B2E', fontSize: '1.6rem', fontWeight: 800, marginBottom: 20 }}>Story Angles</h2>
          <p style={{ color: '#4A4D6B', marginBottom: 24, fontSize: '0.95rem' }}>Suggested directions for journalists and bloggers covering Draw in the Air:</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {[
              ...
              <div key={s.angle} style={{ background: 'rgba(3,78,174,0.051)', border: '1px solid rgba(3,78,174,0.128)', borderRadius: 12, padding: '18px 20px' }}>
                <div style={{ color: '#1A1B2E', fontWeight: 700, fontSize: '0.95rem', marginBottom: 6 }}>{s.angle}</div>
                <div style={{ color: '#4A4D6B', fontSize: '0.88rem', lineHeight: 1.65 }}>{s.desc}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <Section light>
        <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center' }}>
          <h2 style={{ color: '#1A1B2E', fontSize: '1.6rem', fontWeight: 800, marginBottom: 16 }}>Media Contact</h2>
          <p style={{ color: '#4A4D6B', fontSize: '1rem', lineHeight: 1.7, marginBottom: 24 }}>
          ...
          <p style={{ color: '#6B6F84', fontSize: '0.82rem', marginTop: 16 }}>
            We typically respond within 24 hours.
          </p>
          <div style={{ display: 'flex', justifyContent: 'center', gap: 12, marginTop: 24, flexWrap: 'wrap' }}>
            <button onClick={() => navigate(SITE.appPath)} style={{ background: 'rgba(3,78,174,0.10)', border: '1px solid rgba(3,78,174,0.255)', color: '#6C3FA4', borderRadius: 20, padding: '8px 18px', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer' }}>
              Try the Platform
            </button>
            <button onClick={() => navigate('/schools')} style={{ background: 'rgba(85,221,224,0.16)', border: '1px solid rgba(28,126,128,0.25)', color: '#1c7e80', borderRadius: 20, padding: '8px 18px', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer' }}>
              School Information
            </button>
